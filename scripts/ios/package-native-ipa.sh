...
app_path="${1:?usage: package-native-ipa.sh <app-path> <output-directory>}"
output_directory="${2:?usage: package-native-ipa.sh <app-path> <output-directory>}"
...
entitlements_path="$output_directory/EvaOrbitNative-ad-hoc-entitlements.plist"
ipa_path="$output_directory/EvaOrbitNative-ad-hoc.ipa"
...
ditto "$app_path" "$output_directory/Payload/EvaOrbitNative.app"
(
  cd "$output_directory"
