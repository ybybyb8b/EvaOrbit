...
import type { DrinkInputSuggestions, Pet, TrackerField, TrackerSummary, TrainingInputSuggestions } from "@/lib/types";
...
import { TrainingLogEditor } from "./health/training-log-editor";
...
export function HomeQuickLog({ selectedDate, onSaved }: { selectedDate: string; onSaved: () => Promise<void> }) {
  ...
  const [trainingSuggestions, setTrainingSuggestions] = useState(emptyTrainingSuggestions);
  ...
  async function choose(kind: Exclude<QuickKind, "picker" | "food" | "inbox"> | "food" | "inbox") {
    playNativeHaptic("selection"); setError("");
    if (kind === "food" || kind === "inbox") { setActive(kind); return; }
    setLoading(true);
    try {
      ...
      if (kind === "training") { const response = await fetch("/api/health/training/suggestions", { cache: "no-store" }); if (!response.ok) throw new Error(); setTrainingSuggestions(await response.json()); }
  ...
  const options: Array<{ kind: "food" | "drink" | "training" | "tracker" | "cats" | "inbox"; icon: IconName; en: string; zh: string }> = [
    { kind: "food", icon: "food", en: "Food", zh: "饮食" }, { kind: "drink", icon: "drink", en: "Drink", zh: "饮品" }, { kind: "training", icon: "health", en: "Training", zh: "训练" },
  ...
  return <>
    ...
    {active === "training" && <TrainingLogEditor key={`training-${selectedDate}`} initialDate={selectedDate} suggestions={trainingSuggestions} onClose={close} onSaved={saved} />}
