...
export function HomeTodayBrief({ events }: { events: TimelineEvent[] }) {
  ...
  if (!events.length) return <p className="home-today-empty">No records today.</p>;
  ...
  return <>
    ...
    {events.length > PREVIEW_LIMIT && <button type="button" className="home-today-toggle" onClick={() => setExpanded((value) => !value)}>{expanded ? "Show less" : `View all today · ${events.length}`}</button>}
  </>;
}
