...
import { Archive, ChevronRight, ListCheck } from "reicon-react";
...
export default function DataBackupPage() {
  return <div className="page settings-detail-page data-backup-page"><PageHeader eyebrow="数据与备份" title="Data & Backup" description="查看当前存储方式与已有数据入口。" action={<Link className="settings-back-link" href="/settings">全部设置</Link>} />
    ...
    <BackupExportCard />
    <section className="legacy-data-settings"><div><span className="eyebrow">PRESERVED DATA</span><h2>旧数据还在</h2><p>Task 和 Memory 已从日常导航撤下，但页面、记录和历史链接仍然保留。这里是它们稳定的入口。</p></div><nav aria-label="旧数据入口"><Link href="/tasks"><span><ListCheck size={15} weight="Outline" />旧待办</span><ChevronRight size={15} weight="Outline" /></Link><Link href="/memory"><span><Archive size={15} weight="Outline" />旧 Memory</span><ChevronRight size={15} weight="Outline" /></Link></nav></section>
  </div>;
}
