...
export function FoodLibraryView() {
  ...
  return <div className="page">
    ...
    {items.length ? <div className="food-library-list">{items.map((item) => <article key={item.id}>
      <button type="button" className="food-library-copy" onClick={() => openEdit(item)} aria-label={`Edit ${item.name}`}><span>{item.brand || "No brand"}</span><h2>{item.name}</h2><p>{[item.defaultPortion, item.referenceKcal !== null ? `${item.referenceKcal} kcal` : "", item.servingKcal !== null ? `${item.servingKcal} kcal / serving` : ""].filter(Boolean).join(" · ") || "No serving data"}</p></button>
