...
export async function GET(request:NextRequest){try{return NextResponse.json(await listRelationPeople({query:request.nextUrl.searchParams.get("q")||undefined,includeArchived:request.nextUrl.searchParams.get("archived")==="true"}));}catch(error){return apiError(error);}}
