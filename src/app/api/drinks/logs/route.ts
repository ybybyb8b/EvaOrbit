...
export async function GET(request:NextRequest){try{const p=request.nextUrl.searchParams;return NextResponse.json(await listDrinkLogs({date:p.get("date")||undefined,drinkType:p.get("drinkType")||undefined}));}catch(error){return apiError(error);}}
