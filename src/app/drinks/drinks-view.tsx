...
export function DrinksView(){
  ...
  return <div className="page drinks-page">
    <PageHeader eyebrow="LIFE" title="Drinks" description="从真实饮用记录里看见习惯，也保留每一杯的原始事实。" action={<button className="button primary" onClick={openCreate}><Icon name="plus"/>补一杯</button>}/>
    <section className="drink-preference-section"><div className="section-heading"><div><span className="eyebrow">HISTORY & PREFERENCES</span><h2>我的饮品习惯</h2></div><Link className="text-button" href="/drinks/history">查看全部历史 <Icon name="arrow"/></Link></div>
    ...
    <div className="section-heading drink-today-heading"><div><span className="eyebrow">TODAY</span><h2>今天喝了什么</h2></div></div>
    {logs.length?<div className="drink-record-list">{logs.map(log=><DrinkRecordCard log={log} onEdit={openEdit} key={log.id}/>)}</div>:<div className="empty-state drink-empty-state"><h2>今天还没记饮品</h2><p>需要时直接补一杯。</p></div>}
  </div>;
}
