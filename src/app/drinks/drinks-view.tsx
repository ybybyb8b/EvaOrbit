...
export function DrinksView() {
  ...
  async function submit(event: FormEvent) {
    ...
    const response = await fetch(editing ? `/api/drinks/logs/${editing}` : "/api/drinks/logs", { method: editing ? "PATCH" : "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) });
    if (!response.ok) { setError(((await response.json()) as ApiError).error); return; }
    setDraft(emptyDrink); setEditing(null); setShowForm(false); await load();
  }
