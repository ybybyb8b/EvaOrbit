...
function emptyDraft(initialDate?:string):Draft{return{occurredAt:initialDate??currentLocalDate(),name:"",brand:"",drinkType:"other",volumeMl:"",sugarLevel:"",temperature:"",rating:"",estimatedKcal:"",kcalMin:"",kcalMax:"",confidence:"medium",notes:""};}
function draftFromRecord(record?:DrinkLog,initialDate?:string):Draft{return record?{occurredAt:compactDateTimeValue(record.occurredAt,record.occurredHasExplicitTime),name:record.name,brand:record.brand,drinkType:record.drinkType,volumeMl:record.volumeMl?.toString()??"",sugarLevel:editableSugarLevel(record.sugarLevel),temperature:record.temperature??"",rating:record.rating??"",estimatedKcal:record.estimatedKcal?.toString()??"",kcalMin:record.kcalMin?.toString()??"",kcalMax:record.kcalMax?.toString()??"",confidence:record.confidence,notes:record.notes}:emptyDraft(initialDate);}

export function DrinkRecordEditor({record,suggestions,initialDate,onClose,onSaved,onDeleted}:{record?:DrinkLog;suggestions:DrinkInputSuggestions;initialDate?:string;onClose:()=>void;onSaved:()=>Promise<void>|void;onDeleted?:()=>Promise<void>|void}){
