...
import { formatRecordMoment, healthRecordStatusLabels, healthRecordTypeLabels, healthRecordUsesStatus } from "./health-record-utils";
...
export function HealthRecordDetailView({ initial }: { initial: HealthRecord }) {
  ...
  return <div className="page health-page health-detail-page">
    ...
    {!editing && <>
      ...
      <div className="health-detail-actions"><button className="button primary" onClick={() => setEditing(true)}><Icon name="edit" />Edit</button>{healthRecordUsesStatus(record.type) && <button className="button secondary" onClick={() => void changeStatus()}>{record.status === "active" ? "Resolve" : "Reopen"}</button>}<button className="text-button danger" onClick={() => void remove()}><Icon name="trash" />Delete</button></div>
    </>}
  </div>;
}
