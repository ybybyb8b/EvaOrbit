...
export function InboxCaptureForm({ onSaved, onCapture, formId, sheet = false }: { onSaved: () => Promise<void> | void; onCapture?: (content: string) => Promise<void> | void; formId?: string; sheet?: boolean }) {
  const { english } = useLocale();
  const [content, setContent] = useState("");
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState("");
  async function submit(event: FormEvent) { event.preventDefault(); if (saving) return; setSaving(true); setError(""); try { if(onCapture)await onCapture(content);else{const response = await fetch("/api/inbox", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ content, source: "manual" }) }); if (!response.ok) { setError(((await response.json()) as ApiError).error); return; }} setContent(""); await onSaved(); } catch(reason){setError(reason instanceof Error?reason.message:"Could not save Inbox item.");} finally { setSaving(false); } }
