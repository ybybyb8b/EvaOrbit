...
export function buildSubscriptionTimelineEvents(payments:SubscriptionPayment[],subscriptions:Subscription[]):TimelineEvent[]{const names=new Map(subscriptions.map(item=>[item.id,item.name]));return payments.map(payment=>({id:`subscription:${payment.id}`,eventType:"subscription.paid",sourceType:"subscription" as const,sourceId:payment.id,title:names.get(payment.subscriptionId)??"Subscription",detail:`${payment.currency} ${(payment.amountMinor/100).toFixed(2)}${payment.note?` · ${payment.note}`:""}`,occurredAt:`${payment.paidOn}T12:00:00.000Z`,hasExplicitTime:false,endAt:null,href:`/subscriptions/${payment.subscriptionId}`,relatedPeople:[],relatedPets:[],metadata:{subscriptionId:payment.subscriptionId,amountMinor:payment.amountMinor,currency:payment.currency,scheduledFor:payment.scheduledFor}})).sort(compareTimelineEvents);}
