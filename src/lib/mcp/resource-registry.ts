...
export type ResourceField = {
  type: "integer" | "number" | "string" | "boolean" | "array" | "object" | "json";
  description: string;
  format?: string;
...
export type ResourceRegistryOperations = {
  memoryEntity: {search(input:{query?:string;entityType?:string;status?:MemoryEntity["status"];includeMerged?:boolean;limit?:number}):Promise<MemoryEntity[]>;get(id:string):Promise<MemoryEntityDetail|null>;create(input:ReturnType<typeof parseNewMemoryEntity>):Promise<MemoryEntity>;update(id:string,input:ReturnType<typeof parseMemoryEntityPatch>):Promise<MemoryEntity|null>;setArchived(id:string,archived:boolean):Promise<MemoryEntity|null>;merge(sourceId:string,targetId:string):Promise<MemoryEntityMergeResult|null>};
  ...
  luciusPostComment?: { search(input:{postId?:number;author?:LuciusPostComment["author"];limit?:number}):Promise<LuciusPostComment[]>; get(id:number):Promise<LuciusPostComment|null>; create(input:Omit<LuciusPostComment,"id"|"createdAt"|"updatedAt">):Promise<LuciusPostComment>; update(id:number,input:Pick<LuciusPostComment,"content">):Promise<LuciusPostComment|null>; delete(id:number):Promise<boolean> };
  ...
  subscription?: { search(input:{query?:string;status?:Subscription["status"];limit?:number}):Promise<Subscription[]>; get(id:number):Promise<SubscriptionDetail|null>; create(input:ReturnType<typeof parseNewSubscription>):Promise<Subscription>; update(id:number,input:ReturnType<typeof parseSubscriptionPatch>):Promise<Subscription|null>; setStatus(id:number,status:Subscription["status"],nextRenewalOn?:string):Promise<Subscription|null>; recordPayment(id:number,input:ReturnType<typeof parseSubscriptionPayment>):Promise<SubscriptionPayment> };
  foodPlace?: {search(query?:string,options?:{status?:FoodPlace["status"];category?:string;limit?:number}):Promise<FoodPlace[]>;get(id:number):Promise<FoodPlace|null>;create(input:ReturnType<typeof parseFoodPlace>):Promise<FoodPlace>;update(id:number,input:ReturnType<typeof parseFoodPlacePatch>):Promise<FoodPlace|null>};
  foodDish?: {search(query?:string,options?:{foodPlaceId?:number;recommended?:boolean;rating?:FoodDish["rating"];limit?:number}):Promise<FoodDish[]>;get(id:number):Promise<FoodDish|null>;create(input:ReturnType<typeof parseFoodDish>):Promise<FoodDish>;update(id:number,input:ReturnType<typeof parseFoodDishPatch>):Promise<FoodDish|null>;delete(id:number):Promise<boolean>};
};
...
function uuidId(value:ResourceId,resource="Resource"){try{return memoryUuid(value,`${resource} id`);}catch{throw new ValidationError(`${resource} id must be a UUID.`);}}
...
function luciusPostCommentRecord(item:LuciusPostComment):ResourceRecord{return{id:item.id,post_id:item.postId,author:item.author,content:item.content,created_at:item.createdAt,updated_at:item.updatedAt};}
function luciusPostCommentResource(operations:NonNullable<ResourceRegistryOperations["luciusPostComment"]>):RegisteredResource{return{schema:{resource:"lucius_post_comment",description:"A chronological conversation attached to one Lucius post. Use author=lucius when replying as Lucius and author=user only for the user's own words.",fields:{id:{type:"integer",description:"Comment id.",read_only:true},post_id:{type:"integer",description:"Parent Lucius Post id."},author:{type:"string",enum:["user","lucius"],description:"Visible speaker identity."},content:{type:"string",max_length:2000,description:"Comment body."},...timestamps},required_fields:["post_id","author","content"],writable_fields:["post_id","author","content"],searchable_fields:["post_id","author","content"],supported_actions:[],validation_rules:["search may filter by post_id and author","results are chronological","post_id and author cannot be changed after creation","use author=lucius for an assistant reply; never attribute invented text to the user","do not edit or delete a user-authored comment unless the user explicitly requests it","unknown fields are rejected"]},async search({query,filters={},limit,cursor}){assertOnlyKeys(filters,["post_id","author"],"lucius_post_comment search filters");rejectCursor(cursor,"Lucius Post comment");const postId=filters.post_id===undefined?undefined:numericId(filters.post_id as ResourceId,"Lucius Post");const author=filterEnum(filters.author,["user","lucius"] as const,"author");let items=await operations.search({postId,author,limit:Math.min(limit*3,500)});if(query)items=items.filter(item=>item.content.toLocaleLowerCase().includes(query.toLocaleLowerCase()));return{items:items.slice(0,limit).map(luciusPostCommentRecord),next_cursor:null};},async get(value){const item=await operations.get(numericId(value,"Lucius Post comment"));if(!item)throw new ConflictError("Lucius Post comment not found.");return luciusPostCommentRecord(item);},async create(data){assertOnlyKeys(data,["post_id","author","content"],"lucius_post_comment create");return luciusPostCommentRecord(await operations.create(parseNewLuciusPostComment(mappedInput(data,{post_id:"postId",author:"author",content:"content"}))));},async update(value,data){assertOnlyKeys(data,["content"],"lucius_post_comment update");const item=await operations.update(numericId(value,"Lucius Post comment"),parseLuciusPostCommentPatch(data));if(!item)throw new ConflictError("Lucius Post comment not found.");return luciusPostCommentRecord(item);},async delete(value){const id=numericId(value,"Lucius Post comment");if(!await operations.delete(id))throw new ConflictError("Lucius Post comment not found.");return{deleted:true,id};}};}
...
const routineMap={scope:"scope",pet_id:"petId",title:"title",interval_value:"intervalValue",interval_unit:"intervalUnit",first_due_at:"firstDueAt",next_due_at:"nextDueAt",reminder_lead_minutes:"reminderLeadMinutes",repeat_while_overdue:"repeatWhileOverdue",notes:"notes",enabled:"enabled"};
...
const foodDishMap={food_place_id:"foodPlaceId",name:"name",category:"category",rating:"rating",recommended:"recommended",notes:"notes"};
function foodDishResource(operations:NonNullable<ResourceRegistryOperations["foodDish"]>):RegisteredResource{const writable=Object.keys(foodDishMap);return{schema:{resource:"food_dish",description:"A long-term dish note belonging to one specific food place.",fields:{id:{type:"integer",description:"Dish id.",read_only:true},food_place_id:{type:"integer",description:"Owning food place id."},name:{type:"string",max_length:200,description:"Dish name."},category:{type:"string",max_length:100,description:"Optional free-form category."},rating:{type:"string",enum:["love","good","neutral","dislike"],description:"Long-term dish rating."},recommended:{type:"boolean",description:"Whether the dish is recommended."},notes:{type:"string",max_length:4000,description:"Notes."},eat_count:{type:"integer",description:"Derived linked Food Record count.",read_only:true},last_eaten_at:{type:"string",format:"date-time",description:"Derived latest linked Food Record time.",read_only:true},created_at:{type:"string",format:"date-time",description:"Created time.",read_only:true},updated_at:{type:"string",format:"date-time",description:"Updated time.",read_only:true}},required_fields:["food_place_id","name"],writable_fields:writable,searchable_fields:["name","category","notes"],supported_actions:[],validation_rules:["each dish belongs to exactly one food_place","category is free-form","eat_count and last_eaten_at are derived from Food Records","delete archives a dish when Food Records reference it"]},async search({query,filters={},limit,cursor}){assertOnlyKeys(filters,["food_place_id","recommended","rating"],"food_dish search filters");rejectCursor(cursor,"Food dish");const foodPlaceId=filters.food_place_id===undefined?undefined:numericId(filters.food_place_id as ResourceId,"Food place");if(filters.recommended!==undefined&&typeof filters.recommended!=="boolean")throw new ValidationError("recommended filter is invalid.");const recommended=filters.recommended as boolean|undefined;const rating=filterEnum(filters.rating,["love","good","neutral","dislike"] as const,"rating");return{items:(await operations.search(query??"",{foodPlaceId,recommended,rating,limit})).map(snakeRecord),next_cursor:null};},async get(value){const item=await operations.get(numericId(value,"Food dish"));if(!item)throw new ConflictError("Food dish not found.");return snakeRecord(item);},async create(data){assertOnlyKeys(data,writable,"food_dish create");return snakeRecord(await operations.create(parseFoodDish(mappedInput(data,foodDishMap))));},async update(value,data){assertOnlyKeys(data,writable,"food_dish update");const item=await operations.update(numericId(value,"Food dish"),parseFoodDishPatch(mappedInput(data,foodDishMap)));if(!item)throw new ConflictError("Food dish not found.");return snakeRecord(item);},async delete(value){const id=numericId(value,"Food dish");if(!await operations.delete(id))throw new ConflictError("Food dish not found.");return{deleted:true,id};}};}

function memoryEntityRecord(item:MemoryEntity):ResourceRecord{return{id:item.id,canonical_name:item.canonicalName,entity_type:item.entityType,aliases:item.aliases,description:item.description,status:item.status,merged_into_entity_id:item.mergedIntoEntityId,created_at:item.createdAt,updated_at:item.updatedAt};}
function memoryFactRecord(item:MemoryFact):ResourceRecord{return{id:item.id,subject_entity_id:item.subjectEntityId,predicate:item.predicate,object_entity_id:item.objectEntityId,object_value:item.objectValue,perspective_entity_id:item.perspectiveEntityId,epistemic_type:item.epistemicType,supersedes_fact_id:item.supersedesFactId,confidence:item.confidence,importance:item.importance,valid_from:item.validFrom,valid_to:item.validTo,status:item.status,invalidated_at:item.invalidatedAt,invalidation_reason:item.invalidationReason,created_at:item.createdAt,updated_at:item.updatedAt};}
function memorySourceRecord(item:MemorySource):ResourceRecord{return{id:item.id,fact_id:item.factId,source_resource:item.sourceResource,source_record_id:item.sourceRecordId,source_url:item.sourceUrl,excerpt:item.excerpt,note:item.note,created_at:item.createdAt,updated_at:item.updatedAt};}

function memoryEntityResource(operations:ResourceRegistryOperations["memoryEntity"]):RegisteredResource{const writable=["canonical_name","entity_type","aliases","description"];return{schema:{resource:"memory_entity",description:"A durable, manually resolved entity in EvaOrbit Memory Graph.",fields:{id:{type:"string",format:"uuid",description:"Application-generated UUID.",read_only:true},canonical_name:{type:"string",max_length:200,description:"Primary display name; not globally unique."},entity_type:{type:"string",max_length:80,description:"Loose stable type such as person, identity, project, place, or concept."},aliases:{type:"array",items:{type:"string"},description:"Alternative names used by manual entity resolution."},description:{type:"string",max_length:5000,description:"Optional durable description."},status:{type:"string",enum:["active","archived","merged"],description:"Server-managed lifecycle state.",read_only:true},merged_into_entity_id:{type:"string",format:"uuid",description:"Redirect target for a merged entity.",read_only:true},created_at:{type:"string",format:"date-time",description:"Created timestamp.",read_only:true},updated_at:{type:"string",format:"date-time",description:"Updated timestamp.",read_only:true}},required_fields:["canonical_name","entity_type"],writable_fields:writable,searchable_fields:["canonical_name","aliases","entity_type","status"],supported_actions:["merge","archive","restore"],validation_rules:["canonical_name is not unique","entity_type is intentionally open-ended","search excludes merged redirects unless include_merged is true or status is explicit","merge atomically redirects subject, object, and perspective references and retains the old entity","ordinary delete is unsupported","unknown fields are rejected"]},async search({query,filters={},limit,cursor}){rejectCursor(cursor,"Memory Entity");assertOnlyKeys(filters,["entity_type","status","include_merged"],"memory_entity search filters");const parsed=parseMemoryEntitySearch(mappedInput(filters,{entity_type:"entityType",status:"status",include_merged:"includeMerged"}));return{items:(await operations.search({query,...parsed,limit})).map(memoryEntityRecord),next_cursor:null};},async get(value){const detail=await operations.get(uuidId(value,"Memory Entity"));if(!detail)throw new ConflictError("Memory Entity not found.");return{...memoryEntityRecord(detail.entity),facts:detail.facts.map(memoryFactRecord)};},async create(data){assertOnlyKeys(data,writable,"memory_entity create");return memoryEntityRecord(await operations.create(parseNewMemoryEntity(mappedInput(data,{canonical_name:"canonicalName",entity_type:"entityType",aliases:"aliases",description:"description"}))));},async update(value,data){assertOnlyKeys(data,writable,"memory_entity update");const item=await operations.update(uuidId(value,"Memory Entity"),parseMemoryEntityPatch(mappedInput(data,{canonical_name:"canonicalName",entity_type:"entityType",aliases:"aliases",description:"description"})));if(!item)throw new ConflictError("Memory Entity not found or already merged.");return memoryEntityRecord(item);},async action({id,action,data}){if(id===undefined)throw new ValidationError(`${action} requires an entity id.`);const entityId=uuidId(id,"Memory Entity");if(action==="merge"){assertOnlyKeys(data,["target_entity_id"],"memory_entity merge");const item=await operations.merge(entityId,uuidId(data.target_entity_id as ResourceId,"Target Memory Entity"));if(!item)throw new ConflictError("Memory Entity not found.");return{...memoryEntityRecord(item.entity),redirected_facts:item.redirectedFacts,self_loops:item.selfLoops};}if(action==="archive"||action==="restore"){assertOnlyKeys(data,[],`memory_entity ${action}`);const item=await operations.setArchived(entityId,action==="archive");if(!item)throw new ConflictError("Memory Entity not found or cannot change state.");return memoryEntityRecord(item);}throw new ValidationError(`memory_entity does not support action: ${action}.`);}};}

function memoryFactResource(operations:ResourceRegistryOperations["memoryFact"]):RegisteredResource{const createFields=["subject_entity_id","predicate","object_entity_id","object_value","perspective_entity_id","epistemic_type","confidence","importance","valid_from","valid_to"],updateFields=["confidence","importance","valid_from","valid_to"],map={subject_entity_id:"subjectEntityId",predicate:"predicate",object_entity_id:"objectEntityId",object_value:"objectValue",perspective_entity_id:"perspectiveEntityId",epistemic_type:"epistemicType",confidence:"confidence",importance:"importance",valid_from:"validFrom",valid_to:"validTo"};return{schema:{resource:"memory_fact",description:"A durable directed relation or literal assertion in EvaOrbit Memory Graph.",fields:{id:{type:"string",format:"uuid",description:"Application-generated UUID.",read_only:true},subject_entity_id:{type:"string",format:"uuid",description:"Subject entity."},predicate:{type:"string",max_length:120,description:"Open-ended stable predicate."},object_entity_id:{type:"string",format:"uuid",description:"Entity object; mutually exclusive with object_value."},object_value:{type:"json",description:"JSON literal; mutually exclusive with object_entity_id."},perspective_entity_id:{type:"string",format:"uuid",description:"Optional assertion perspective, never access control."},epistemic_type:{type:"string",enum:["direct_statement","recorded_observation","derived","agent_judgment","external_report","unknown"],description:"How the assertion was formed."},supersedes_fact_id:{type:"string",format:"uuid",description:"Previous Fact replaced by this assertion.",read_only:true},confidence:{type:"number",description:"Confidence from 0 through 1."},importance:{type:"integer",description:"Importance from 1 through 5."},valid_from:{type:"string",format:"date",description:"Optional real-world validity start."},valid_to:{type:"string",format:"date",description:"Optional real-world validity end; independent of invalidation."},status:{type:"string",enum:["active","invalidated"],description:"Server-managed graph assertion lifecycle.",read_only:true},invalidated_at:{type:"string",format:"date-time",description:"When this graph assertion was invalidated.",read_only:true},invalidation_reason:{type:"string",description:"Optional invalidation reason.",read_only:true},created_at:{type:"string",format:"date-time",description:"Created timestamp.",read_only:true},updated_at:{type:"string",format:"date-time",description:"Updated timestamp.",read_only:true}},required_fields:["subject_entity_id","predicate","exactly one of object_entity_id or object_value"],writable_fields:createFields,searchable_fields:["entity_id","direction","predicate","perspective_entity_id","status","valid_on"],supported_actions:["invalidate","restore"],validation_rules:["exactly one object form is required","perspective is attribution, not visibility or knowledge","subject, predicate, object, and perspective cannot be updated in place","use the reviewed candidate path to supersede a fact","valid_to is real-world validity and is never changed by invalidation","ordinary delete is unsupported","unknown fields are rejected"]},async search({query,filters={},limit,cursor}){rejectCursor(cursor,"Memory Fact");assertOnlyKeys(filters,["entity_id","direction","predicate","perspective_entity_id","status","valid_on"],"memory_fact search filters");const parsed=parseMemoryFactSearch(mappedInput(filters,{entity_id:"entityId",direction:"direction",predicate:"predicate",perspective_entity_id:"perspectiveEntityId",status:"status",valid_on:"validOn"}));let items=await operations.search({...parsed,limit});if(query){const needle=query.toLocaleLowerCase();items=items.filter(item=>`${item.predicate} ${JSON.stringify(item.objectValue)}`.toLocaleLowerCase().includes(needle));}return{items:items.map(memoryFactRecord),next_cursor:null};},async get(value){const detail=await operations.get(uuidId(value,"Memory Fact"));if(!detail)throw new ConflictError("Memory Fact not found.");return{...memoryFactRecord(detail.fact),sources:detail.sources.map(memorySourceRecord)};},async create(data){assertOnlyKeys(data,createFields,"memory_fact create");return memoryFactRecord(await operations.create(parseNewMemoryFact(mappedInput(data,map))));},async update(value,data){assertOnlyKeys(data,updateFields,"memory_fact update");const item=await operations.update(uuidId(value,"Memory Fact"),parseMemoryFactPatch(mappedInput(data,map)));if(!item)throw new ConflictError("Memory Fact not found.");return memoryFactRecord(item);},async action({id,action,data}){if(id===undefined)throw new ValidationError(`${action} requires a fact id.`);const factId=uuidId(id,"Memory Fact");if(action==="invalidate"){assertOnlyKeys(data,["reason"],"memory_fact invalidate");const reason=data.reason===undefined||data.reason===null?null:String(data.reason).trim();if(reason&&reason.length>2000)throw new ValidationError("invalidation reason is too long.");const item=await operations.invalidate(factId,reason||null);if(!item)throw new ConflictError("Memory Fact not found or already invalidated.");return memoryFactRecord(item);}if(action==="restore"){assertOnlyKeys(data,[],"memory_fact restore");const item=await operations.restore(factId);if(!item)throw new ConflictError("Memory Fact not found or already active.");return memoryFactRecord(item);}throw new ValidationError(`memory_fact does not support action: ${action}.`);}};}

function memorySourceResource(operations:ResourceRegistryOperations["memorySource"]):RegisteredResource{const createFields=["fact_id","source_resource","source_record_id","source_url","excerpt","note"],updateFields=["source_url","excerpt","note"],map={fact_id:"factId",source_resource:"sourceResource",source_record_id:"sourceRecordId",source_url:"sourceUrl",excerpt:"excerpt",note:"note"};return{schema:{resource:"memory_source",description:"Provenance linking one Memory Fact to an EvaOrbit resource or manual, chat, or external source.",fields:{id:{type:"string",format:"uuid",description:"Application-generated UUID.",read_only:true},fact_id:{type:"string",format:"uuid",description:"Owning Memory Fact."},source_resource:{type:"string",max_length:100,description:"Resource Registry key or manual, chat, or external."},source_record_id:{type:"string",max_length:300,description:"Opaque source identifier; never coerced to a number."},source_url:{type:"string",format:"uri",max_length:2000,description:"Optional HTTP or HTTPS URL."},excerpt:{type:"string",max_length:10000,description:"Optional source excerpt retained when the source changes or disappears."},note:{type:"string",max_length:2000,description:"Optional provenance note."},created_at:{type:"string",format:"date-time",description:"Created timestamp.",read_only:true},updated_at:{type:"string",format:"date-time",description:"Updated timestamp.",read_only:true}},required_fields:["fact_id","source_resource","at least one of source_record_id, source_url, excerpt, or note"],writable_fields:createFields,searchable_fields:["fact_id","source_resource","source_record_id"],supported_actions:[],validation_rules:["source_record_id is always an opaque string","source_resource is open-ended and has no business-table foreign key","update changes only URL, excerpt, or note","delete removes only the provenance link, never the Fact","unknown fields are rejected"]},async search({filters={},limit,cursor}){rejectCursor(cursor,"Memory Source");assertOnlyKeys(filters,["fact_id","source_resource","source_record_id"],"memory_source search filters");if(filters.source_record_id!==undefined&&typeof filters.source_record_id!=="string")throw new ValidationError("source_record_id must be an opaque string.");const factId=filters.fact_id===undefined?undefined:uuidId(filters.fact_id as ResourceId,"Memory Fact"),sourceResource=filters.source_resource===undefined?undefined:filterText(filters.source_resource,"source_resource"),sourceRecordId=filters.source_record_id===undefined?undefined:filterText(filters.source_record_id,"source_record_id");return{items:(await operations.search({factId,sourceResource,sourceRecordId,limit})).map(memorySourceRecord),next_cursor:null};},async get(value){const item=await operations.get(uuidId(value,"Memory Source"));if(!item)throw new ConflictError("Memory Source not found.");return memorySourceRecord(item);},async create(data){assertOnlyKeys(data,createFields,"memory_source create");return memorySourceRecord(await operations.create(parseNewMemorySource(mappedInput(data,map))));},async update(value,data){assertOnlyKeys(data,updateFields,"memory_source update");const item=await operations.update(uuidId(value,"Memory Source"),parseMemorySourcePatch(mappedInput(data,map)));if(!item)throw new ConflictError("Memory Source not found.");return memorySourceRecord(item);},async delete(value){const id=uuidId(value,"Memory Source");if(!await operations.delete(id))throw new ConflictError("Memory Source not found.");return{deleted:true,id};}};}
...
export function createResourceRegistry(operations: ResourceRegistryOperations) {
  const resources:RegisteredResource[]=[
    memoryEntityResource(operations.memoryEntity),
  ...
  if(operations.subscription)resources.push(subscriptionResource(operations.subscription));
  if(operations.foodPlace)resources.push(foodPlaceResource(operations.foodPlace));
  if(operations.foodDish)resources.push(foodDishResource(operations.foodDish));
