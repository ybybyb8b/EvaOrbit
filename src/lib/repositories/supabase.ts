...
import type { AiModelConfig, AiProvider, CatEvent, CatMeasurement, CatMedication, CatRoutine, CatSymptom, CatVetVisit, ChatMessage, ChatRole, ChatSession, DrinkLimit, DrinkLog, FoodLibraryItem, FoodLog, HealthRecord, InboxItem, Memory, NotificationDelivery, Pet, PushSubscriptionRecord, Reminder, ReminderOccurrence, Task, TaskPriority, Tracker, TrackerEntry, TrackerField, TrackerGoal, TrackerReminder } from "../types";
import type { AiModelConfigInput, AiProviderInput, AiSettingsInput, EvaOrbitRepository, FoodLibrarySearchOptions, HealthRecordListInput, InternalAiProvider, InternalAiSettings, NewTask, TaskFilter } from "./types";
...
function healthRecordFromRow(row: Row): HealthRecord { return { id: Number(row.id), occurredAt: String(row.occurred_at), type: row.type as HealthRecord["type"], title: String(row.title), summary: String(row.summary ?? ""), status: row.status as HealthRecord["status"], startedAt: row.started_at ? String(row.started_at) : null, endedAt: row.ended_at ? String(row.ended_at) : null, details: healthRecordDetailsFromRow(row.details), createdAt: String(row.created_at), updatedAt: String(row.updated_at) }; }
...
function buildSupabaseRepository(client: SupabaseClient, userId: string): EvaOrbitRepository {
  ...
  const repository: EvaOrbitRepository = {
    ...
    async createHealthRecord(input){const{data,error}=await client.from("health_records").insert({user_id:userId,occurred_at:input.occurredAt,type:input.type,title:input.title,summary:input.summary,status:input.status,started_at:input.startedAt,ended_at:input.endedAt,details:input.details}).select().single();fail(error,"创建健康记录");return healthRecordFromRow(data);},
    async updateHealthRecord(id,input){const patch=mappedPatch(input,{occurredAt:"occurred_at",type:"type",title:"title",summary:"summary",status:"status",startedAt:"started_at",endedAt:"ended_at",details:"details"});if(!Object.keys(patch).length)return repository.getHealthRecord(id);const{data,error}=await client.from("health_records").update(patch).eq("id",id).select().maybeSingle();fail(error,"更新健康记录");return data?healthRecordFromRow(data):null;},
    ...
    async createCatEvent(input){const{data,error}=await client.from("cat_events").insert({user_id:userId,pet_id:input.petId,event_type:input.eventType,occurred_at:input.occurredAt,title:input.title,note:input.note,source_type:input.sourceType,source_id:input.sourceId}).select().single();fail(error,"创建猫咪护理记录");return catEventFromRow(data);},
    async updateCatEvent(id,input){return catUpdate("cat_events",id,input,{petId:"pet_id",eventType:"event_type",occurredAt:"occurred_at",title:"title",note:"note",sourceType:"source_type",sourceId:"source_id"},catEventFromRow);},
    ...
    async createCatSymptom(input){const{data,error}=await client.from("cat_symptoms").insert({user_id:userId,pet_id:input.petId,occurred_at:input.occurredAt,title:input.title,severity:input.severity,description:input.description,body_area:input.bodyArea,note:input.note}).select().single();fail(error,"创建症状记录");return catSymptomFromRow(data);},
    async updateCatSymptom(id,input){return catUpdate("cat_symptoms",id,input,{petId:"pet_id",occurredAt:"occurred_at",title:"title",severity:"severity",description:"description",bodyArea:"body_area",note:"note"},catSymptomFromRow);},
    ...
    async createCatMedication(input){const{data,error}=await client.from("cat_medications").insert({user_id:userId,pet_id:input.petId,name:input.name,dose:input.dose,unit:input.unit,frequency_text:input.frequencyText,started_at:input.startedAt,ended_at:input.endedAt,reason:input.reason,active:input.active,notes:input.notes}).select().single();fail(error,"创建用药记录");return catMedicationFromRow(data);},
    async updateCatMedication(id,input){return catUpdate("cat_medications",id,input,{petId:"pet_id",name:"name",dose:"dose",unit:"unit",frequencyText:"frequency_text",startedAt:"started_at",endedAt:"ended_at",reason:"reason",active:"active",notes:"notes"},catMedicationFromRow);},
    ...
    async createCatMeasurement(input){const{data,error}=await client.from("cat_measurements").insert({user_id:userId,pet_id:input.petId,occurred_at:input.occurredAt,measurement_type:input.measurementType,value:input.value,unit:input.unit,note:input.note}).select().single();fail(error,"创建测量记录");return catMeasurementFromRow(data);},
    async updateCatMeasurement(id,input){return catUpdate("cat_measurements",id,input,{petId:"pet_id",occurredAt:"occurred_at",measurementType:"measurement_type",value:"value",unit:"unit",note:"note"},catMeasurementFromRow);},
