...
import type { AiModelConfig, AiProvider, CatEvent, CatMeasurement, CatMedication, CatRoutine, CatSymptom, CatVetVisit, ChatMessage, ChatRole, ChatSession, ChronicleEntry, DrinkLimit, DrinkLog, FoodDish, FoodLibraryItem, FoodLog, FoodPlace, HealthRecord, InboxItem, LuciusCase, LuciusDiaryEntry, LuciusPost, LuciusPostComment, LuciusState, MealReminderRule, MediaItem, MediaSeries, MediaViewing, Memo, Memory, NotificationDelivery, PersonMemoryNote, Pet, Project, ProjectItem, PushSubscriptionRecord, RelationEvent, RelationPerson, Reminder, ReminderOccurrence, Task, TaskPriority, Tracker, TrackerEntry, TrackerField, TrackerGoal, TrackerReminder, TrainingLog } from "../types";
import type { AiModelConfigInput, AiProviderInput, AiSettingsInput, ChronicleEntryPatch, EvaOrbitRepository, FoodLibrarySearchOptions, HealthRecordListInput, InternalAiProvider, InternalAiSettings, LuciusCasePatch, LuciusDiaryPatch, LuciusPostCommentPatch, LuciusPostPatch, LuciusStatePatch, MediaItemPatch, MemoPatch, NewTask, ProjectItemPatch, ProjectPatch, TaskFilter } from "./types";
...
function luciusPostCommentFromRow(row:Row):LuciusPostComment{return{id:Number(row.id),postId:Number(row.post_id),author:row.author as LuciusPostComment["author"],content:String(row.content),createdAt:String(row.created_at),updatedAt:String(row.updated_at)};}
...
function trackerReminderFromRow(row:Row):TrackerReminder{return{id:Number(row.id),trackerId:Number(row.tracker_id),reminderMode:(row.notification_mode??(row.reminder_type==="scheduled"?"standard":"missing")) as TrackerReminder["reminderMode"],configuredTime:String(row.configured_time??row.schedule_rule??"20:00").slice(0,5),periodDays:Number(row.period_days??row.interval_days??1),anchorDate:String(row.anchor_date),nextDueAt:String(row.next_due_at),timezone:String(row.timezone??"Asia/Shanghai"),reminderId:row.reminder_id===null||row.reminder_id===undefined?null:Number(row.reminder_id),enabled:Boolean(row.enabled),createdAt:String(row.created_at),updatedAt:String(row.updated_at)};}
...
function mealReminderRuleFromRow(row:Row):MealReminderRule{return{mealType:row.meal_type as MealReminderRule["mealType"],remindAt:String(row.remind_at).slice(0,5),enabled:Boolean(row.enabled),updatedAt:String(row.updated_at)};}
...
function buildSupabaseRepository(client: SupabaseClient, userId: string): EvaOrbitRepository {
  ...
  const repository: EvaOrbitRepository = {
    ...
    async getUiPreferences() {
      const { data, error } = await client.from("ui_preferences").select("home_module_order,appearance_mode,color_theme,ui_language,chinese_font,english_font,updated_at").maybeSingle();
      fail(error, "Read UI preferences");
      return { homeModuleOrder: normalizeHomeModuleOrder(data?.home_module_order), appearanceMode: normalizeAppearanceMode(data?.appearance_mode), colorTheme: normalizeColorTheme(data?.color_theme), uiLanguage: normalizeUiLanguage(data?.ui_language), chineseFont: normalizeChineseFont(data?.chinese_font), englishFont: normalizeEnglishFont(data?.english_font), updatedAt: data?.updated_at ? String(data.updated_at) : "" };
    },
    async updateHomeModuleOrder(order: HomeModuleId[]) {
      const { data, error } = await client.from("ui_preferences").upsert({ user_id: userId, home_module_order: normalizeHomeModuleOrder(order) }, { onConflict: "user_id" }).select("home_module_order,appearance_mode,color_theme,ui_language,chinese_font,english_font,updated_at").single();
      fail(error, "Save home module order");
      if (!data) throw new Error("Save home module order failed");
      return { homeModuleOrder: normalizeHomeModuleOrder(data.home_module_order), appearanceMode: normalizeAppearanceMode(data.appearance_mode), colorTheme: normalizeColorTheme(data.color_theme), uiLanguage: normalizeUiLanguage(data.ui_language), chineseFont: normalizeChineseFont(data.chinese_font), englishFont: normalizeEnglishFont(data.english_font), updatedAt: String(data.updated_at) };
    },
    async updateAppearancePreferences(input) {
      const values = { user_id: userId, appearance_mode: input.appearanceMode, color_theme: input.colorTheme, ui_language: input.uiLanguage, chinese_font: input.chineseFont, english_font: input.englishFont };
      const { data, error } = await client.from("ui_preferences").upsert(values, { onConflict: "user_id" }).select("home_module_order,appearance_mode,color_theme,ui_language,chinese_font,english_font,updated_at").single();
    ...
    async listLuciusPostComments(input={}){if(input.postIds&&!input.postIds.length)return[];let request=client.from("lucius_post_comments").select("*");if(input.postId!==undefined)request=request.eq("post_id",input.postId);if(input.postIds)request=request.in("post_id",input.postIds);if(input.author)request=request.eq("author",input.author);const limit=Math.min(Math.max(input.limit??200,1),500);const{data,error}=await request.order("created_at",{ascending:true}).order("id",{ascending:true}).limit(limit);fail(error,"读取 Lucius Post 评论");return(data as Row[]).map(luciusPostCommentFromRow);},
    async getLuciusPostComment(id){const{data,error}=await client.from("lucius_post_comments").select("*").eq("id",id).maybeSingle();fail(error,"读取 Lucius Post 评论");return data?luciusPostCommentFromRow(data):null;},
    async createLuciusPostComment(input){const{data,error}=await client.from("lucius_post_comments").insert({user_id:userId,post_id:input.postId,author:input.author,content:input.content}).select().single();fail(error,"创建 Lucius Post 评论");return luciusPostCommentFromRow(data);},
    async updateLuciusPostComment(id,input:LuciusPostCommentPatch){const{data,error}=await client.from("lucius_post_comments").update({content:input.content}).eq("id",id).select().maybeSingle();fail(error,"更新 Lucius Post 评论");return data?luciusPostCommentFromRow(data):null;},
    async deleteLuciusPostComment(id){const{data,error}=await client.from("lucius_post_comments").delete().eq("id",id).select("id").maybeSingle();fail(error,"删除 Lucius Post 评论");return Boolean(data);},
    ...
    async getTrackerReminder(id){const{data,error}=await client.from("tracker_reminders").select("*").eq("id",id).maybeSingle();fail(error,"读取 Tracker Reminder");return data?trackerReminderFromRow(data):null;},
    async createTrackerReminder(input){const legacyType=input.reminderMode==="standard"?"scheduled":"interval";const{data,error}=await client.from("tracker_reminders").insert({user_id:userId,tracker_id:input.trackerId,reminder_type:legacyType,schedule_rule:input.configuredTime,interval_days:input.periodDays,notification_mode:input.reminderMode,configured_time:input.configuredTime,period_days:input.periodDays,anchor_date:input.anchorDate,next_due_at:input.nextDueAt,timezone:input.timezone,reminder_id:input.reminderId,enabled:input.enabled}).select().single();fail(error,"创建 Tracker Reminder");return trackerReminderFromRow(data);},
    async updateTrackerReminder(id,input){return catUpdate("tracker_reminders",id,input,{reminderMode:"notification_mode",configuredTime:"configured_time",periodDays:"period_days",anchorDate:"anchor_date",nextDueAt:"next_due_at",timezone:"timezone",reminderId:"reminder_id",enabled:"enabled"},trackerReminderFromRow);},
    ...
    async listMealReminderRules(){const{data,error}=await client.from("meal_reminder_rules").select("meal_type,remind_at,enabled,updated_at").order("remind_at");fail(error,"读取三餐提醒");return(data as Row[]).map(mealReminderRuleFromRow);},
    async updateMealReminderRules(input){const rows=input.map(rule=>({user_id:userId,meal_type:rule.mealType,remind_at:rule.remindAt,enabled:rule.enabled}));const{error}=await client.from("meal_reminder_rules").upsert(rows,{onConflict:"user_id,meal_type"});fail(error,"保存三餐提醒");return repository.listMealReminderRules();},
