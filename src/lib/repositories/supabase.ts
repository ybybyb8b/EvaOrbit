...
function relationPersonFromRow(row:Row):RelationPerson{return{id:Number(row.id),name:String(row.name),nickname:row.nickname?String(row.nickname):null,relationLabel:row.relation_label?String(row.relation_label):null,photoPath:row.photo_path?String(row.photo_path):null,birthday:row.birthday?String(row.birthday):null,likes:row.likes?String(row.likes):null,avoid:row.avoid?String(row.avoid):null,note:row.note?String(row.note):null,archivedAt:row.archived_at?String(row.archived_at):null,createdAt:String(row.created_at),updatedAt:String(row.updated_at)};}
...
function buildSupabaseRepository(client: SupabaseClient, userId: string): EvaOrbitRepository {
  ...
  const repository: EvaOrbitRepository = {
    ...
    async listMediaItems(input={}){let request=client.from("media_items").select("*");if(input.query)request=request.ilike("title",`%${input.query.replace(/[\\%_]/g,"\\$&")}%`);if(input.mediaType)request=request.eq("media_type",input.mediaType);const limit=Math.min(Math.max(input.limit??100,1),200);const{data,error}=await request.order("updated_at",{ascending:false}).order("id",{ascending:false}).limit(limit);fail(error,"读取 Media");return(data as Row[]).map(mediaItemFromRow);},
    async getMediaItem(id){const{data,error}=await client.from("media_items").select("*").eq("id",id).maybeSingle();fail(error,"读取 Media");return data?mediaItemFromRow(data):null;},
    async createMediaItem(input){const{data,error}=await client.from("media_items").insert({user_id:userId,title:input.title,media_type:input.mediaType,rating:input.rating,note:input.note,cover_url:input.coverUrl}).select().single();fail(error,"创建 Media");return mediaItemFromRow(data);},
    async updateMediaItem(id,input:MediaItemPatch){const patch=mappedPatch(input,{title:"title",mediaType:"media_type",rating:"rating",note:"note"});if(!Object.keys(patch).length)return repository.getMediaItem(id);const{data,error}=await client.from("media_items").update(patch).eq("id",id).select().maybeSingle();fail(error,"更新 Media");return data?mediaItemFromRow(data):null;},
