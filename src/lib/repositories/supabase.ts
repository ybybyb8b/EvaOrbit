...
import type { AiModelConfig, AiProvider, CatEvent, CatMeasurement, CatMedication, CatSymptom, CatVetVisit, ChatMessage, ChatRole, ChatSession, DrinkLimit, DrinkLog, FoodLibraryItem, FoodLog, InboxItem, Memory, Pet, PushSubscriptionRecord, Reminder, ReminderOccurrence, Task, TaskPriority, Tracker, TrackerEntry, TrackerField, TrackerGoal, TrackerReminder } from "../types";
...
function buildSupabaseRepository(client: SupabaseClient, userId: string): EvaOrbitRepository {
  ...
  const repository: EvaOrbitRepository = {
    ...
    async searchFoodLibrary(query="",brand=""){let request=client.from("food_library").select("*").is("archived_at",null);if(query)request=request.ilike("name",`%${query.replace(/[\\%_]/g,"\\$&")}%`);if(brand)request=request.eq("brand",brand);const{data,error}=await request.order("updated_at",{ascending:false}).limit(100);fail(error,"搜索 Food Library");return(data as Row[]).map(libraryFromRow);},
