...
function timestampWithExplicitTime(value:unknown,field="时间"){
  if(typeof value!=="string"||!/T\d{2}:\d{2}/.test(value))throw new ValidationError(`${field}必须包含明确日期和时间`);
  return timestamp(value,field);
}
...
const menstrualFlows = ["none", "unspecified", "light", "medium", "heavy"] as const;
function healthId(value:unknown,field:string,nullable=false){if(nullable&&(value===null||value===undefined||value===""))return null;const result=Number(value);if(!Number.isSafeInteger(result)||result<1)throw new ValidationError(`${field}格式不正确`);return result;}
function reminderInterval(value:unknown){const result=Number(value);if(!Number.isSafeInteger(result)||result<1||result>43200)throw new ValidationError("提醒间隔必须是 1 到 43200 分钟");return result;}

export function parseNewMenstrualPeriod(value:unknown){const body=objectValue(value),startedOn=dateOnly(body.startedOn,"经期开始日期"),endedOn=body.endedOn===null||body.endedOn===undefined||body.endedOn===""?null:dateOnly(body.endedOn,"经期结束日期");if(endedOn&&endedOn<startedOn)throw new ValidationError("经期结束日期不能早于开始日期");return{startedOn,endedOn,notes:text(body.notes??"","备注",5000,false)??""};}
export function parseMenstrualPeriodPatch(value:unknown){const body=objectValue(value);const result={startedOn:body.startedOn===undefined?undefined:dateOnly(body.startedOn,"经期开始日期"),endedOn:body.endedOn===undefined?undefined:body.endedOn===null||body.endedOn===""?null:dateOnly(body.endedOn,"经期结束日期"),notes:body.notes===undefined?undefined:text(body.notes,"备注",5000,false)??""};if(Object.values(result).every(item=>item===undefined))throw new ValidationError("没有可更新的经期字段");if(result.startedOn&&result.endedOn&&result.endedOn<result.startedOn)throw new ValidationError("经期结束日期不能早于开始日期");return result;}
export function parseNewMenstrualFlowRecord(value:unknown){const body=objectValue(value),periodId=healthId(body.periodId,"经期 ID",true),isCycleStart=booleanValue(body.isCycleStart,"周期开始标记",false);if(isCycleStart&&periodId===null)throw new ValidationError("周期开始记录必须关联经期");return{periodId,occurredAt:timestamp(body.occurredAt,"经量记录时间"),occurredHasExplicitTime:booleanValue(body.occurredHasExplicitTime,"经量记录时间精度",false),flow:enumValue(body.flow,"经量",menstrualFlows,"unspecified"),isCycleStart,notes:text(body.notes??"","备注",5000,false)??""};}
export function parseMenstrualFlowRecordPatch(value:unknown){const body=objectValue(value);const result={periodId:body.periodId===undefined?undefined:healthId(body.periodId,"经期 ID",true),occurredAt:body.occurredAt===undefined?undefined:timestamp(body.occurredAt,"经量记录时间"),occurredHasExplicitTime:body.occurredHasExplicitTime===undefined?undefined:booleanValue(body.occurredHasExplicitTime,"经量记录时间精度",false),flow:body.flow===undefined?undefined:enumValue(body.flow,"经量",menstrualFlows,"unspecified"),isCycleStart:body.isCycleStart===undefined?undefined:booleanValue(body.isCycleStart,"周期开始标记",false),notes:body.notes===undefined?undefined:text(body.notes,"备注",5000,false)??""};if(Object.values(result).every(item=>item===undefined))throw new ValidationError("没有可更新的经量字段");if(result.isCycleStart===true&&result.periodId===null)throw new ValidationError("周期开始记录必须关联经期");return result;}
export function parseNewMedicationPreset(value:unknown){const body=objectValue(value);return{name:text(body.name,"药物名称",200)!,defaultDoseText:text(body.defaultDoseText??"","默认用量描述",200,false)??"",minReminderIntervalMinutes:reminderInterval(body.minReminderIntervalMinutes),reminderEnabled:booleanValue(body.reminderEnabled,"提醒状态",false),periodLinkEnabled:booleanValue(body.periodLinkEnabled,"经期联动状态",false),notes:text(body.notes??"","备注",5000,false)??""};}
export function parseMedicationPresetPatch(value:unknown){const body=objectValue(value);const result={name:body.name===undefined?undefined:text(body.name,"药物名称",200),defaultDoseText:body.defaultDoseText===undefined?undefined:text(body.defaultDoseText,"默认用量描述",200,false)??"",minReminderIntervalMinutes:body.minReminderIntervalMinutes===undefined?undefined:reminderInterval(body.minReminderIntervalMinutes),reminderEnabled:body.reminderEnabled===undefined?undefined:booleanValue(body.reminderEnabled,"提醒状态",false),periodLinkEnabled:body.periodLinkEnabled===undefined?undefined:booleanValue(body.periodLinkEnabled,"经期联动状态",false),notes:body.notes===undefined?undefined:text(body.notes,"备注",5000,false)??""};if(Object.values(result).every(item=>item===undefined))throw new ValidationError("没有可更新的药物配置字段");return result;}
export function parseNewMedicationDoseEvent(value:unknown){const body=objectValue(value);return{medicationPresetId:healthId(body.medicationPresetId,"药物配置 ID")!,periodId:healthId(body.periodId,"经期 ID",true),takenAt:timestampWithExplicitTime(body.takenAt,"实际服药时间"),doseText:text(body.doseText??"","实际用量描述",200,false)??"",notes:text(body.notes??"","备注",5000,false)??""};}
export function parseMedicationDoseEventPatch(value:unknown){const body=objectValue(value);const result={medicationPresetId:body.medicationPresetId===undefined?undefined:healthId(body.medicationPresetId,"药物配置 ID")!,periodId:body.periodId===undefined?undefined:healthId(body.periodId,"经期 ID",true),takenAt:body.takenAt===undefined?undefined:timestampWithExplicitTime(body.takenAt,"实际服药时间"),doseText:body.doseText===undefined?undefined:text(body.doseText,"实际用量描述",200,false)??"",notes:body.notes===undefined?undefined:text(body.notes,"备注",5000,false)??""};if(Object.values(result).every(item=>item===undefined))throw new ValidationError("没有可更新的服药字段");return result;}
