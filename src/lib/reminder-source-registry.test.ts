...
test("Reminder source registry keeps deterministic and conditional channels separate", () => {
  for (const source of [null, "cat_routine", "vet_visit", "medication", "tracker_standard", "subscription_renewal", "task_due"]) {
  ...
  assert.equal(Object.keys(REMINDER_SOURCE_REGISTRY).length, 11);
});
