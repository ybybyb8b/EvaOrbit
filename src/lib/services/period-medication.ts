...
export async function createMenstrualFlowRecord(input:NewMenstrualFlowRecord){return(await getRepository()).createMenstrualFlowRecord(input);}
...
export async function createMedicationDoseEvent(input:NewMedicationDoseEvent){const item=await(await getRepository()).createMedicationDoseEvent(input);await reconcilePeriodMedicationReminder(item.medicationPresetId);return item;}
