...
import type { AiModelConfig, AiProvider, AiSettings, CatEvent, CatMeasurement, CatMedication, CatSymptom, CatVetVisit, ChatMessage, ChatPreferences, ChatRole, ChatSession, DashboardSummary, DrinkLimit, DrinkLog, FoodLibraryItem, FoodLog, InboxItem, Memory, Pet, PushSubscriptionRecord, Reminder, ReminderOccurrence, Task, Tracker, TrackerEntry, TrackerField, TrackerGoal, TrackerReminder } from "./types";
...
function catEventFromRow(row:Record<string,unknown>):CatEvent{return{id:Number(row.id),petId:row.pet_id===null?null:Number(row.pet_id),eventType:row.event_type as CatEvent["eventType"],occurredAt:String(row.occurred_at),title:String(row.title),note:String(row.note??""),sourceType:row.source_type?String(row.source_type):null,sourceId:row.source_id===null?null:Number(row.source_id),createdAt:String(row.created_at),updatedAt:String(row.updated_at)};}
function catSymptomFromRow(row:Record<string,unknown>):CatSymptom{return{id:Number(row.id),petId:Number(row.pet_id),occurredAt:String(row.occurred_at),title:String(row.title),severity:String(row.severity??""),description:String(row.description??""),bodyArea:String(row.body_area??""),note:String(row.note??""),createdAt:String(row.created_at),updatedAt:String(row.updated_at)};}
function catVetVisitFromRow(row:Record<string,unknown>):CatVetVisit{return{id:Number(row.id),petId:Number(row.pet_id),occurredAt:String(row.occurred_at),clinic:String(row.clinic??""),doctor:String(row.doctor??""),reason:String(row.reason),symptoms:String(row.symptoms??""),diagnosis:String(row.diagnosis??""),examinations:String(row.examinations??""),treatment:String(row.treatment??""),prescriptions:String(row.prescriptions??""),cost:row.cost===null?null:Number(row.cost),followUpAt:row.follow_up_at?String(row.follow_up_at):null,notes:String(row.notes??""),createdAt:String(row.created_at),updatedAt:String(row.updated_at)};}
function catMedicationFromRow(row:Record<string,unknown>):CatMedication{return{id:Number(row.id),petId:Number(row.pet_id),name:String(row.name),dose:String(row.dose??""),unit:String(row.unit??""),frequencyText:String(row.frequency_text??""),startedAt:String(row.started_at),endedAt:row.ended_at?String(row.ended_at):null,reason:String(row.reason??""),active:Boolean(row.active),notes:String(row.notes??""),createdAt:String(row.created_at),updatedAt:String(row.updated_at)};}
function catMeasurementFromRow(row:Record<string,unknown>):CatMeasurement{return{id:Number(row.id),petId:Number(row.pet_id),occurredAt:String(row.occurred_at),measurementType:String(row.measurement_type),value:Number(row.value),unit:String(row.unit),note:String(row.note??""),createdAt:String(row.created_at),updatedAt:String(row.updated_at)};}
function reminderFromRow(row:Record<string,unknown>):Reminder{return{id:Number(row.id),title:String(row.title),targetType:row.target_type as Reminder["targetType"],targetId:row.target_id===null?null:Number(row.target_id),sourceType:row.source_type?String(row.source_type):null,sourceId:row.source_id===null?null:Number(row.source_id),scheduleType:row.schedule_type as Reminder["scheduleType"],startsAt:String(row.starts_at),nextDueAt:row.next_due_at?String(row.next_due_at):null,intervalValue:row.interval_value===null?null:Number(row.interval_value),intervalUnit:row.interval_unit as Reminder["intervalUnit"],timesOfDay:jsonValue(row.times_of_day,[]),endsAt:row.ends_at?String(row.ends_at):null,timezone:String(row.timezone),isActive:Boolean(row.is_active),lastCompletedAt:row.last_completed_at?String(row.last_completed_at):null,snoozedUntil:row.snoozed_until?String(row.snoozed_until):null,lastNotifiedAt:row.last_notified_at?String(row.last_notified_at):null,createdAt:String(row.created_at),updatedAt:String(row.updated_at)};}
...
export function createCatEvent(input:Omit<CatEvent,"id"|"createdAt"|"updatedAt">){const result=database.prepare("INSERT INTO cat_events(pet_id,event_type,occurred_at,title,note,source_type,source_id) VALUES(?,?,?,?,?,?,?)").run(input.petId,input.eventType,input.occurredAt,input.title,input.note,input.sourceType,input.sourceId);return getCatEvent(Number(result.lastInsertRowid))!;}
export function updateCatEvent(id:number,input:Record<string,unknown>){updateCatsRow("cat_events",id,input,{petId:"pet_id",eventType:"event_type",occurredAt:"occurred_at",title:"title",note:"note",sourceType:"source_type",sourceId:"source_id"});return getCatEvent(id);}
...
export function createCatSymptom(input:Omit<CatSymptom,"id"|"createdAt"|"updatedAt">){const r=database.prepare("INSERT INTO cat_symptoms(pet_id,occurred_at,title,severity,description,body_area,note) VALUES(?,?,?,?,?,?,?)").run(input.petId,input.occurredAt,input.title,input.severity,input.description,input.bodyArea,input.note);return getCatSymptom(Number(r.lastInsertRowid))!;}
export function updateCatSymptom(id:number,input:Record<string,unknown>){updateCatsRow("cat_symptoms",id,input,{petId:"pet_id",occurredAt:"occurred_at",title:"title",severity:"severity",description:"description",bodyArea:"body_area",note:"note"});return getCatSymptom(id);}
...
export function createCatMedication(input:Omit<CatMedication,"id"|"createdAt"|"updatedAt">){const r=database.prepare("INSERT INTO cat_medications(pet_id,name,dose,unit,frequency_text,started_at,ended_at,reason,active,notes) VALUES(?,?,?,?,?,?,?,?,?,?)").run(input.petId,input.name,input.dose,input.unit,input.frequencyText,input.startedAt,input.endedAt,input.reason,Number(input.active),input.notes);return getCatMedication(Number(r.lastInsertRowid))!;}
export function updateCatMedication(id:number,input:Record<string,unknown>){updateCatsRow("cat_medications",id,input,{petId:"pet_id",name:"name",dose:"dose",unit:"unit",frequencyText:"frequency_text",startedAt:"started_at",endedAt:"ended_at",reason:"reason",active:"active",notes:"notes"},[],["active"]);return getCatMedication(id);}
...
export function createCatMeasurement(input:Omit<CatMeasurement,"id"|"createdAt"|"updatedAt">){const r=database.prepare("INSERT INTO cat_measurements(pet_id,occurred_at,measurement_type,value,unit,note) VALUES(?,?,?,?,?,?)").run(input.petId,input.occurredAt,input.measurementType,input.value,input.unit,input.note);return getCatMeasurement(Number(r.lastInsertRowid))!;}
export function updateCatMeasurement(id:number,input:Record<string,unknown>){updateCatsRow("cat_measurements",id,input,{petId:"pet_id",occurredAt:"occurred_at",measurementType:"measurement_type",value:"value",unit:"unit",note:"note"});return getCatMeasurement(id);}
...
export function createReminder(input:Omit<Reminder,"id"|"lastCompletedAt"|"snoozedUntil"|"lastNotifiedAt"|"createdAt"|"updatedAt">){const r=database.prepare("INSERT INTO reminders(title,target_type,target_id,source_type,source_id,schedule_type,starts_at,next_due_at,interval_value,interval_unit,times_of_day,ends_at,timezone,is_active) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)").run(input.title,input.targetType,input.targetId,input.sourceType,input.sourceId,input.scheduleType,input.startsAt,input.nextDueAt,input.intervalValue,input.intervalUnit,JSON.stringify(input.timesOfDay),input.endsAt,input.timezone,Number(input.isActive));return getReminder(Number(r.lastInsertRowid))!;}
export function updateReminder(id:number,input:Record<string,unknown>){updateCatsRow("reminders",id,input,{title:"title",targetType:"target_type",targetId:"target_id",sourceType:"source_type",sourceId:"source_id",scheduleType:"schedule_type",startsAt:"starts_at",nextDueAt:"next_due_at",intervalValue:"interval_value",intervalUnit:"interval_unit",timesOfDay:"times_of_day",endsAt:"ends_at",timezone:"timezone",isActive:"is_active",lastCompletedAt:"last_completed_at",snoozedUntil:"snoozed_until",lastNotifiedAt:"last_notified_at"},["timesOfDay"],["isActive"]);return getReminder(id);}
