...
export function createReminder(input:Omit<Reminder,"id"|"lastCompletedAt"|"snoozedUntil"|"lastNotifiedAt"|"sentAt"|"cancelledAt"|"createdAt"|"updatedAt">){const r=database.prepare("INSERT INTO reminders(title,target_type,target_id,source_type,source_id,schedule_type,starts_at,next_due_at,interval_value,interval_unit,times_of_day,ends_at,timezone,note,lead_time_minutes,status,is_active) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)").run(input.title,input.targetType,input.targetId,input.sourceType,input.sourceId,input.scheduleType,input.startsAt,input.nextDueAt,input.intervalValue,input.intervalUnit,JSON.stringify(input.timesOfDay),input.endsAt,input.timezone,input.note,input.leadTimeMinutes,input.status,Number(input.isActive));return getReminder(Number(r.lastInsertRowid))!;}
export function updateReminder(id:number,input:Record<string,unknown>){updateCatsRow("reminders",id,input,{title:"title",targetType:"target_type",targetId:"target_id",sourceType:"source_type",sourceId:"source_id",scheduleType:"schedule_type",startsAt:"starts_at",nextDueAt:"next_due_at",intervalValue:"interval_value",intervalUnit:"interval_unit",timesOfDay:"times_of_day",endsAt:"ends_at",timezone:"timezone",note:"note",leadTimeMinutes:"lead_time_minutes",status:"status",sentAt:"sent_at",cancelledAt:"cancelled_at",isActive:"is_active",lastCompletedAt:"last_completed_at",snoozedUntil:"snoozed_until",lastNotifiedAt:"last_notified_at"},["timesOfDay"],["isActive"]);return getReminder(id);}
