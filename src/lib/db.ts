...
export function createMemoryFact(input:NewMemoryFact){database.prepare("INSERT INTO memory_facts(id,user_id,subject_entity_id,predicate,object_entity_id,object_value,perspective_entity_id,confidence,importance,valid_from,valid_to,status) VALUES(?,'local',?,?,?,?,?,?,?,?,?,?)").run(input.id,input.subjectEntityId,input.predicate,input.objectEntityId,input.objectValue===null?null:JSON.stringify(input.objectValue),input.perspectiveEntityId,input.confidence,input.importance,input.validFrom,input.validTo,input.status);return getMemoryFact(input.id)!;}
...
export function restoreMemoryFact(id:string){const result=database.prepare("UPDATE memory_facts SET status='active',invalidated_at=NULL,invalidation_reason=NULL,updated_at=CURRENT_TIMESTAMP WHERE id=? AND user_id='local' AND status='invalidated'").run(id);return result.changes?getMemoryFact(id):null;}
