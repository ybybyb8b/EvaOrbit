...
const hasV25 = database.prepare("SELECT 1 FROM migrations WHERE version = 25").get();
if (!hasV25) database.exec(`
  BEGIN;
  ALTER TABLE food_logs ADD COLUMN rating TEXT CHECK(rating IS NULL OR rating IN ('love','good','neutral','dislike'));
  ALTER TABLE drink_logs ADD COLUMN occurred_has_explicit_time INTEGER NOT NULL DEFAULT 1 CHECK(occurred_has_explicit_time IN (0,1));
  ALTER TABLE drink_logs ADD COLUMN temperature TEXT CHECK(temperature IS NULL OR temperature IN ('normal_ice','less_ice','no_ice','room_temperature','hot'));
  ALTER TABLE drink_logs ADD COLUMN rating TEXT CHECK(rating IS NULL OR rating IN ('love','good','neutral','dislike'));
  INSERT INTO migrations(version) VALUES(25);
  COMMIT;
`);

const hasV26 = database.prepare("SELECT 1 FROM migrations WHERE version = 26").get();
if (!hasV26) database.exec(`
  BEGIN;
  ...
  CREATE UNIQUE INDEX idx_media_series_user_name ON media_series(user_id,name COLLATE NOCASE);
  ALTER TABLE media_items ADD COLUMN status TEXT NOT NULL DEFAULT 'completed' CHECK(status IN ('planned','watching','completed','paused','dropped'));
  ALTER TABLE media_items ADD COLUMN is_favorite INTEGER NOT NULL DEFAULT 0 CHECK(is_favorite IN (0,1));
  ALTER TABLE media_items ADD COLUMN series_id INTEGER REFERENCES media_series(id) ON DELETE RESTRICT;
  ALTER TABLE media_items ADD COLUMN season_number INTEGER CHECK(season_number IS NULL OR season_number BETWEEN 1 AND 999);
  ALTER TABLE media_items ADD COLUMN season_title TEXT CHECK(season_title IS NULL OR length(season_title) <= 120);
  CREATE INDEX idx_media_items_user_shelf ON media_items(user_id,status,media_type,is_favorite,updated_at DESC);
  CREATE INDEX idx_media_items_user_series ON media_items(user_id,series_id,season_number,id);
  INSERT INTO migrations(version) VALUES(26);
  COMMIT;
`);

const hasV27 = database.prepare("SELECT 1 FROM migrations WHERE version = 27").get();
if (!hasV27) database.exec(`
  BEGIN;
  ALTER TABLE media_items ADD COLUMN original_title TEXT CHECK(original_title IS NULL OR length(trim(original_title)) BETWEEN 1 AND 300);
  ALTER TABLE media_items ADD COLUMN translated_title TEXT CHECK(translated_title IS NULL OR length(trim(translated_title)) BETWEEN 1 AND 300);
  CREATE INDEX idx_media_items_user_original_title ON media_items(user_id,original_title);
  CREATE INDEX idx_media_items_user_translated_title ON media_items(user_id,translated_title);
  INSERT INTO migrations(version) VALUES(27);
  COMMIT;
`);

const hasV28 = database.prepare("SELECT 1 FROM migrations WHERE version = 28").get();
if (!hasV28) database.exec(`
  BEGIN;
  ALTER TABLE relation_people ADD COLUMN closeness_rank INTEGER CHECK(closeness_rank IS NULL OR closeness_rank BETWEEN 1 AND 5);
  ALTER TABLE relation_people ADD COLUMN relationship_status TEXT NOT NULL DEFAULT 'active' CHECK(relationship_status IN ('active','ended'));
  CREATE INDEX relation_people_status_closeness ON relation_people(user_id,archived_at,relationship_status,closeness_rank DESC,name);
...
const hasV30 = database.prepare("SELECT 1 FROM migrations WHERE version = 30").get();
if (!hasV30) database.exec(`
  BEGIN;
  CREATE TABLE lucius_posts (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    user_id TEXT NOT NULL DEFAULT 'local',
  ...
  CREATE INDEX lucius_posts_user_published ON lucius_posts(user_id,published_at DESC,id DESC);
...
const hasV36 = database.prepare("SELECT 1 FROM migrations WHERE version = 36").get();
if (!hasV36) database.exec(`
  BEGIN;
  CREATE TABLE meal_reminder_rules (
    meal_type TEXT PRIMARY KEY CHECK(meal_type IN ('breakfast','lunch','dinner')),
    remind_at TEXT NOT NULL CHECK(remind_at GLOB '[0-2][0-9]:[0-5][0-9]'),
    enabled INTEGER NOT NULL DEFAULT 1 CHECK(enabled IN (0,1)),
    updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP
  );
  INSERT INTO meal_reminder_rules(meal_type,remind_at,enabled) VALUES
    ('breakfast','10:00',1),('lunch','14:00',1),('dinner','20:00',1);
  CREATE UNIQUE INDEX idx_meal_notification_once ON notification_deliveries(target_type,target_id,scheduled_at) WHERE source_type='meal_missing';
  INSERT INTO migrations(version) VALUES(36);
  COMMIT;
`);

const hasV37 = database.prepare("SELECT 1 FROM migrations WHERE version = 37").get();
if (!hasV37) database.exec(`
  BEGIN;
  ALTER TABLE cat_routines ADD COLUMN recurrence_mode TEXT NOT NULL DEFAULT 'completion' CHECK(recurrence_mode IN ('completion','fixed'));
  ALTER TABLE cat_routines ADD COLUMN anchor_date TEXT;
  ALTER TABLE cat_routines ADD COLUMN first_due_date TEXT;
  ALTER TABLE cat_routines ADD COLUMN next_due_date TEXT;
  ALTER TABLE cat_routines ADD COLUMN configured_reminder_time TEXT;
  ALTER TABLE cat_routines ADD COLUMN timezone TEXT NOT NULL DEFAULT 'Asia/Shanghai';
  UPDATE cat_routines SET anchor_date=date(first_due_at,'+8 hours'),first_due_date=date(first_due_at,'+8 hours'),next_due_date=date(next_due_at,'+8 hours'),configured_reminder_time=strftime('%H:%M',first_due_at,'+8 hours');
  ALTER TABLE tracker_reminders ADD COLUMN notification_mode TEXT NOT NULL DEFAULT 'missing' CHECK(notification_mode IN ('standard','missing'));
  ALTER TABLE tracker_reminders ADD COLUMN configured_time TEXT NOT NULL DEFAULT '20:00';
  ALTER TABLE tracker_reminders ADD COLUMN period_days INTEGER NOT NULL DEFAULT 1 CHECK(period_days>0);
  ALTER TABLE tracker_reminders ADD COLUMN anchor_date TEXT NOT NULL DEFAULT '1970-01-01';
  ALTER TABLE tracker_reminders ADD COLUMN next_due_at TEXT;
  ALTER TABLE tracker_reminders ADD COLUMN timezone TEXT NOT NULL DEFAULT 'Asia/Shanghai';
  ALTER TABLE tracker_reminders ADD COLUMN reminder_id INTEGER REFERENCES reminders(id) ON DELETE SET NULL;
  UPDATE tracker_reminders SET notification_mode=CASE WHEN reminder_type='scheduled' THEN 'standard' ELSE 'missing' END,configured_time=CASE WHEN schedule_rule GLOB '[0-2][0-9]:[0-5][0-9]' THEN schedule_rule ELSE configured_time END,period_days=COALESCE(interval_days,1),anchor_date=date('now','+8 hours');
  UPDATE tracker_reminders SET next_due_at=strftime('%Y-%m-%dT%H:%M:%fZ',anchor_date || ' ' || configured_time,printf('+%d days',CASE WHEN notification_mode='missing' THEN period_days-1 ELSE 0 END),'-8 hours');
  INSERT INTO reminders(title,target_type,target_id,source_type,source_id,schedule_type,starts_at,next_due_at,due_has_explicit_time,interval_value,interval_unit,times_of_day,timezone,note,lead_time_minutes,status,is_active)
    SELECT t.name,'tracker',tr.tracker_id,'tracker_' || tr.notification_mode,tr.id,'interval',tr.next_due_at,tr.next_due_at,1,tr.period_days,'day','[]',tr.timezone,'',0,CASE WHEN tr.enabled=1 THEN 'scheduled' ELSE 'cancelled' END,tr.enabled FROM tracker_reminders tr JOIN trackers t ON t.id=tr.tracker_id WHERE tr.reminder_id IS NULL;
  UPDATE tracker_reminders SET reminder_id=(SELECT id FROM reminders WHERE source_id=tracker_reminders.id AND source_type IN ('tracker_standard','tracker_missing') ORDER BY id DESC LIMIT 1) WHERE reminder_id IS NULL;
  CREATE UNIQUE INDEX idx_tracker_notification_once ON notification_deliveries(source_type,source_id,scheduled_at) WHERE source_type IN ('tracker_standard','tracker_missing');
...
const hasV39 = database.prepare("SELECT 1 FROM migrations WHERE version = 39").get();
if (!hasV39) database.exec(`
  BEGIN;
  CREATE TABLE memory_entities (
    id TEXT PRIMARY KEY,user_id TEXT NOT NULL DEFAULT 'local',canonical_name TEXT NOT NULL CHECK(length(trim(canonical_name)) BETWEEN 1 AND 200),
    entity_type TEXT NOT NULL CHECK(length(trim(entity_type)) BETWEEN 1 AND 80),aliases TEXT NOT NULL DEFAULT '[]' CHECK(json_valid(aliases) AND json_type(aliases)='array' AND json_array_length(aliases)<=50),
    description TEXT CHECK(description IS NULL OR length(description)<=5000),status TEXT NOT NULL DEFAULT 'active' CHECK(status IN ('active','archived','merged')),
    merged_into_entity_id TEXT,created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,UNIQUE(id,user_id),
    FOREIGN KEY(merged_into_entity_id,user_id) REFERENCES memory_entities(id,user_id) ON DELETE RESTRICT,
    CHECK(merged_into_entity_id IS NULL OR merged_into_entity_id<>id),CHECK((status='merged')=(merged_into_entity_id IS NOT NULL))
  );
  CREATE TABLE memory_facts (
    id TEXT PRIMARY KEY,user_id TEXT NOT NULL DEFAULT 'local',subject_entity_id TEXT NOT NULL,predicate TEXT NOT NULL CHECK(length(trim(predicate)) BETWEEN 1 AND 120),
    object_entity_id TEXT,object_value TEXT CHECK(object_value IS NULL OR (json_valid(object_value) AND json_type(object_value)<>'null')),perspective_entity_id TEXT,
    confidence REAL NOT NULL DEFAULT 1 CHECK(confidence BETWEEN 0 AND 1),importance INTEGER NOT NULL DEFAULT 3 CHECK(typeof(importance)='integer' AND importance BETWEEN 1 AND 5),
    valid_from TEXT,valid_to TEXT,status TEXT NOT NULL DEFAULT 'active' CHECK(status IN ('active','invalidated')),invalidated_at TEXT,invalidation_reason TEXT CHECK(invalidation_reason IS NULL OR length(invalidation_reason)<=2000),
    created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,UNIQUE(id,user_id),
    FOREIGN KEY(subject_entity_id,user_id) REFERENCES memory_entities(id,user_id) ON DELETE RESTRICT,
    FOREIGN KEY(object_entity_id,user_id) REFERENCES memory_entities(id,user_id) ON DELETE RESTRICT,
    FOREIGN KEY(perspective_entity_id,user_id) REFERENCES memory_entities(id,user_id) ON DELETE RESTRICT,
    CHECK((object_entity_id IS NULL)<>(object_value IS NULL)),CHECK(valid_from IS NULL OR valid_to IS NULL OR valid_to>=valid_from),
  ...
  CREATE TABLE memory_sources (
    id TEXT PRIMARY KEY,user_id TEXT NOT NULL DEFAULT 'local',fact_id TEXT NOT NULL,source_resource TEXT NOT NULL CHECK(length(trim(source_resource)) BETWEEN 1 AND 100),
    source_record_id TEXT CHECK(source_record_id IS NULL OR length(source_record_id) BETWEEN 1 AND 300),source_url TEXT CHECK(source_url IS NULL OR length(source_url)<=2000),excerpt TEXT CHECK(excerpt IS NULL OR length(excerpt)<=10000),note TEXT CHECK(note IS NULL OR length(note)<=2000),created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,UNIQUE(id,user_id),
    FOREIGN KEY(fact_id,user_id) REFERENCES memory_facts(id,user_id) ON DELETE CASCADE,
    CHECK(source_record_id IS NOT NULL OR source_url IS NOT NULL OR excerpt IS NOT NULL OR note IS NOT NULL)
  );
  CREATE INDEX idx_memory_entities_list ON memory_entities(user_id,status,entity_type,updated_at DESC);
  CREATE INDEX idx_memory_entities_name ON memory_entities(user_id,canonical_name COLLATE NOCASE);
  CREATE INDEX idx_memory_facts_subject ON memory_facts(user_id,subject_entity_id,status,updated_at DESC);
  CREATE INDEX idx_memory_facts_object ON memory_facts(user_id,object_entity_id,status,updated_at DESC) WHERE object_entity_id IS NOT NULL;
  CREATE INDEX idx_memory_facts_perspective ON memory_facts(user_id,perspective_entity_id,status,updated_at DESC) WHERE perspective_entity_id IS NOT NULL;
  CREATE INDEX idx_memory_sources_fact ON memory_sources(user_id,fact_id,created_at,id);
  CREATE INDEX idx_memory_sources_origin ON memory_sources(user_id,source_resource,source_record_id);
  CREATE UNIQUE INDEX idx_memory_sources_unique_record ON memory_sources(user_id,fact_id,source_resource,source_record_id) WHERE source_record_id IS NOT NULL;
  INSERT INTO migrations(version) VALUES(39);
  COMMIT;
`);

const hasV40 = database.prepare("SELECT 1 FROM migrations WHERE version = 40").get();
if (!hasV40) database.exec(`
  BEGIN;
  CREATE TABLE weight_records (
    id INTEGER PRIMARY KEY AUTOINCREMENT,user_id TEXT NOT NULL DEFAULT 'local',occurred_at TEXT NOT NULL,
    occurred_has_explicit_time INTEGER NOT NULL DEFAULT 1 CHECK(occurred_has_explicit_time IN(0,1)),
    weight_kg REAL NOT NULL CHECK(weight_kg BETWEEN 20 AND 500),source TEXT NOT NULL DEFAULT 'manual' CHECK(source IN('manual','apple_health')),
    healthkit_sample_id TEXT,healthkit_source_bundle TEXT,healthkit_source_name TEXT,healthkit_sync_identifier TEXT,
    healthkit_sync_version INTEGER NOT NULL DEFAULT 1 CHECK(healthkit_sync_version>=1),created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP
  );
  CREATE INDEX idx_weight_records_user_occurred ON weight_records(user_id,occurred_at DESC,id DESC);
  CREATE UNIQUE INDEX idx_weight_records_healthkit_sample ON weight_records(user_id,healthkit_sample_id) WHERE healthkit_sample_id IS NOT NULL;
  CREATE UNIQUE INDEX idx_weight_records_healthkit_sync ON weight_records(user_id,healthkit_sync_identifier) WHERE healthkit_sync_identifier IS NOT NULL;
  CREATE TABLE weight_settings (user_id TEXT PRIMARY KEY NOT NULL DEFAULT 'local',target_weight_kg REAL,reminder_enabled INTEGER NOT NULL DEFAULT 0 CHECK(reminder_enabled IN(0,1)),reminder_time TEXT NOT NULL DEFAULT '08:00',updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP);
  INSERT INTO weight_settings(user_id) VALUES('local');
  INSERT INTO migrations(version) VALUES(40);
  COMMIT;
`);

const hasV41 = database.prepare("SELECT 1 FROM migrations WHERE version = 41").get();
if (!hasV41) database.exec(`
  BEGIN;
  ALTER TABLE notification_deliveries ADD COLUMN dedupe_key TEXT;
  ALTER TABLE reminders ADD COLUMN repeat_while_overdue INTEGER NOT NULL DEFAULT 0 CHECK(repeat_while_overdue IN (0,1));
  ALTER TABLE cat_routines ADD COLUMN repeat_while_overdue INTEGER NOT NULL DEFAULT 0 CHECK(repeat_while_overdue IN (0,1));
  CREATE UNIQUE INDEX idx_notification_delivery_dedupe ON notification_deliveries(dedupe_key) WHERE dedupe_key IS NOT NULL;
...
const hasV45 = database.prepare("SELECT 1 FROM migrations WHERE version = 45").get();
if (!hasV45) database.exec(`
  BEGIN;
  ALTER TABLE menstrual_flow_records ADD COLUMN ended_at TEXT;
  ALTER TABLE menstrual_flow_records ADD COLUMN source TEXT NOT NULL DEFAULT 'manual' CHECK(source IN('manual','apple_health'));
  ALTER TABLE menstrual_flow_records ADD COLUMN healthkit_sample_id TEXT;
  ALTER TABLE menstrual_flow_records ADD COLUMN healthkit_source_bundle TEXT;
  ALTER TABLE menstrual_flow_records ADD COLUMN healthkit_source_name TEXT;
  ALTER TABLE menstrual_flow_records ADD COLUMN healthkit_sync_identifier TEXT;
  ALTER TABLE menstrual_flow_records ADD COLUMN healthkit_sync_version INTEGER NOT NULL DEFAULT 1 CHECK(healthkit_sync_version>=1);
  ALTER TABLE menstrual_flow_records ADD COLUMN healthkit_sync_status TEXT NOT NULL DEFAULT 'pending' CHECK(healthkit_sync_status IN('pending','synced','pending_delete'));
  ALTER TABLE menstrual_flow_records ADD COLUMN deleted_at TEXT;
  UPDATE menstrual_flow_records
    SET ended_at=occurred_at,
        healthkit_sync_identifier='evaorbit.menstrual_flow.' || lower(hex(randomblob(4))) || '-' || lower(hex(randomblob(2))) || '-4' || substr(lower(hex(randomblob(2))),2) || '-a' || substr(lower(hex(randomblob(2))),2) || '-' || lower(hex(randomblob(6)));
  CREATE UNIQUE INDEX idx_menstrual_flow_healthkit_sample ON menstrual_flow_records(user_id,healthkit_sample_id) WHERE healthkit_sample_id IS NOT NULL;
  CREATE UNIQUE INDEX idx_menstrual_flow_healthkit_sync ON menstrual_flow_records(user_id,healthkit_sync_identifier) WHERE healthkit_sync_identifier IS NOT NULL;
  CREATE INDEX idx_menstrual_flow_healthkit_pending ON menstrual_flow_records(user_id,healthkit_sync_status,updated_at) WHERE healthkit_sync_status<>'synced';
  INSERT INTO migrations(version) VALUES(45);
  COMMIT;
`);

const hasV46 = database.prepare("SELECT 1 FROM migrations WHERE version = 46").get();
if (!hasV46) {
  database.exec("PRAGMA foreign_keys = OFF");
  try {
    database.exec(`
      BEGIN;
      CREATE TABLE reminders_v46 (
        id INTEGER PRIMARY KEY AUTOINCREMENT,title TEXT NOT NULL,target_type TEXT NOT NULL CHECK(target_type IN ('cat','cat_household','tracker','health','subscription')),target_id INTEGER,source_type TEXT,source_id INTEGER,
        schedule_type TEXT NOT NULL CHECK(schedule_type IN ('one_time','interval','course')),starts_at TEXT NOT NULL,next_due_at TEXT,due_has_explicit_time INTEGER NOT NULL DEFAULT 1 CHECK(due_has_explicit_time IN (0,1)),interval_value INTEGER,interval_unit TEXT,times_of_day TEXT NOT NULL DEFAULT '[]',ends_at TEXT,timezone TEXT NOT NULL DEFAULT 'Asia/Shanghai',note TEXT NOT NULL DEFAULT '',lead_time_minutes INTEGER NOT NULL DEFAULT 0 CHECK(lead_time_minutes BETWEEN 0 AND 525600),repeat_while_overdue INTEGER NOT NULL DEFAULT 0 CHECK(repeat_while_overdue IN (0,1)),status TEXT NOT NULL DEFAULT 'scheduled' CHECK(status IN ('scheduled','sent','cancelled','failed','completed')),is_active INTEGER NOT NULL DEFAULT 1 CHECK(is_active IN (0,1)),last_completed_at TEXT,snoozed_until TEXT,last_notified_at TEXT,sent_at TEXT,cancelled_at TEXT,created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP
      );
      INSERT INTO reminders_v46 SELECT * FROM reminders;
      DROP TABLE reminders;
      ALTER TABLE reminders_v46 RENAME TO reminders;
      CREATE INDEX idx_reminders_due ON reminders(is_active,next_due_at);
      CREATE INDEX idx_reminders_status_due ON reminders(status,is_active,next_due_at);
      CREATE UNIQUE INDEX idx_reminders_period_medication_projection ON reminders(source_type,source_id) WHERE source_type='period_medication';
      ...
      CREATE TABLE subscriptions (
        id INTEGER PRIMARY KEY AUTOINCREMENT,user_id TEXT NOT NULL DEFAULT 'local',name TEXT NOT NULL CHECK(length(trim(name)) BETWEEN 1 AND 200),current_amount_minor INTEGER NOT NULL CHECK(current_amount_minor>=0),currency TEXT NOT NULL CHECK(length(currency)=3),billing_interval_value INTEGER NOT NULL CHECK(billing_interval_value>0),billing_interval_unit TEXT NOT NULL CHECK(billing_interval_unit IN('day','week','month','year')),started_on TEXT NOT NULL,next_renewal_on TEXT NOT NULL,status TEXT NOT NULL DEFAULT 'active' CHECK(status IN('active','paused','ended')),auto_renew INTEGER NOT NULL DEFAULT 1 CHECK(auto_renew IN(0,1)),reminder_enabled INTEGER NOT NULL DEFAULT 0 CHECK(reminder_enabled IN(0,1)),reminder_days_before INTEGER NOT NULL DEFAULT 3 CHECK(reminder_days_before BETWEEN 0 AND 3650),reminder_time TEXT,reminder_id INTEGER UNIQUE REFERENCES reminders(id) ON DELETE SET NULL,notes TEXT NOT NULL DEFAULT '' CHECK(length(notes)<=5000),paused_at TEXT,ended_at TEXT,created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,UNIQUE(id,user_id)
      );
      CREATE TABLE subscription_payments (
        id INTEGER PRIMARY KEY AUTOINCREMENT,user_id TEXT NOT NULL DEFAULT 'local',subscription_id INTEGER NOT NULL,scheduled_for TEXT NOT NULL,paid_on TEXT NOT NULL,amount_minor INTEGER NOT NULL CHECK(amount_minor>=0),currency TEXT NOT NULL CHECK(length(currency)=3),note TEXT NOT NULL DEFAULT '' CHECK(length(note)<=5000),created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,FOREIGN KEY(subscription_id,user_id) REFERENCES subscriptions(id,user_id) ON DELETE CASCADE,UNIQUE(user_id,subscription_id,scheduled_for)
      );
      CREATE TABLE subscription_price_changes (
        id INTEGER PRIMARY KEY AUTOINCREMENT,user_id TEXT NOT NULL DEFAULT 'local',subscription_id INTEGER NOT NULL,effective_on TEXT NOT NULL,old_amount_minor INTEGER NOT NULL CHECK(old_amount_minor>=0),new_amount_minor INTEGER NOT NULL CHECK(new_amount_minor>=0),old_currency TEXT NOT NULL,new_currency TEXT NOT NULL,created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,FOREIGN KEY(subscription_id,user_id) REFERENCES subscriptions(id,user_id) ON DELETE CASCADE
      );
      CREATE INDEX idx_subscriptions_status_due ON subscriptions(status,next_renewal_on);
      CREATE INDEX idx_subscription_payments_subscription ON subscription_payments(subscription_id,paid_on DESC,id DESC);
      CREATE INDEX idx_subscription_price_changes_subscription ON subscription_price_changes(subscription_id,effective_on DESC,id DESC);
      INSERT INTO migrations(version) VALUES(46);
      COMMIT;
    `);
  } catch (error) {
    try { database.exec("ROLLBACK"); } catch {}
...
const hasV48 = database.prepare("SELECT 1 FROM migrations WHERE version = 48").get();
if (!hasV48) database.exec(`
  BEGIN;
  ALTER TABLE memory_facts ADD COLUMN epistemic_type TEXT NOT NULL DEFAULT 'unknown' CHECK(epistemic_type IN ('direct_statement','recorded_observation','derived','agent_judgment','external_report','unknown'));
  ALTER TABLE memory_facts ADD COLUMN supersedes_fact_id TEXT REFERENCES memory_facts(id) ON DELETE RESTRICT;
  CREATE UNIQUE INDEX idx_memory_facts_supersedes ON memory_facts(user_id,supersedes_fact_id) WHERE supersedes_fact_id IS NOT NULL;
  CREATE TABLE memory_fact_candidates (
    id TEXT PRIMARY KEY,user_id TEXT NOT NULL DEFAULT 'local',proposed_fact TEXT NOT NULL CHECK(json_valid(proposed_fact) AND json_type(proposed_fact)='object'),
    proposed_sources TEXT NOT NULL CHECK(json_valid(proposed_sources) AND json_type(proposed_sources)='array' AND json_array_length(proposed_sources)>0),
    proposed_by TEXT NOT NULL CHECK(proposed_by IN ('user','model','system','import')),proposer_model TEXT,
    status TEXT NOT NULL DEFAULT 'pending' CHECK(status IN ('pending','promoted','rejected')),review_note TEXT CHECK(review_note IS NULL OR length(review_note)<=2000),
    promoted_fact_id TEXT,created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,reviewed_at TEXT,
    FOREIGN KEY(promoted_fact_id,user_id) REFERENCES memory_facts(id,user_id) ON DELETE RESTRICT,
  ...
  CREATE INDEX idx_memory_fact_candidates_status ON memory_fact_candidates(user_id,status,created_at DESC,id);
  INSERT INTO migrations(version) VALUES(48);
  COMMIT;
`);

const hasV49 = database.prepare("SELECT 1 FROM migrations WHERE version = 49").get();
if (!hasV49) database.exec(`
  BEGIN;
  ALTER TABLE reminders ADD COLUMN overdue_after TEXT;
  CREATE TABLE task_reminders (
    id INTEGER PRIMARY KEY AUTOINCREMENT,user_id TEXT NOT NULL DEFAULT 'local',task_id INTEGER NOT NULL REFERENCES tasks(id) ON DELETE CASCADE,
    reminder_id INTEGER UNIQUE REFERENCES reminders(id) ON DELETE SET NULL,trigger_type TEXT NOT NULL CHECK(trigger_type IN ('absolute','relative')),
    absolute_date TEXT,absolute_time TEXT,relative_to TEXT CHECK(relative_to IN ('due')),offset_minutes INTEGER,timezone TEXT NOT NULL DEFAULT 'Asia/Shanghai',
    repeat_while_overdue INTEGER NOT NULL DEFAULT 0 CHECK(repeat_while_overdue IN (0,1)),created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,
    CHECK((trigger_type='absolute' AND absolute_date IS NOT NULL AND absolute_time IS NOT NULL AND relative_to IS NULL AND offset_minutes IS NULL) OR (trigger_type='relative' AND absolute_date IS NULL AND absolute_time IS NULL AND relative_to='due' AND offset_minutes IS NOT NULL))
  );
  CREATE INDEX idx_task_reminders_task ON task_reminders(user_id,task_id,id);
  CREATE UNIQUE INDEX idx_task_reminder_projection ON reminders(source_type,source_id) WHERE source_type='task_reminder';
  INSERT INTO task_reminders(task_id,reminder_id,trigger_type,relative_to,offset_minutes,timezone,repeat_while_overdue)
    SELECT t.id,t.reminder_id,'relative','due',0,coalesce(r.timezone,'Asia/Shanghai'),coalesce(r.repeat_while_overdue,0) FROM tasks t JOIN reminders r ON r.id=t.reminder_id;
  UPDATE reminders SET source_type='task_reminder',source_id=(SELECT id FROM task_reminders WHERE reminder_id=reminders.id),overdue_after=(SELECT next_due_at FROM reminders source WHERE source.id=reminders.id) WHERE id IN (SELECT reminder_id FROM task_reminders);
  INSERT INTO migrations(version) VALUES(49);
  COMMIT;
`);
...
function weightRecordFromRow(row:Record<string,unknown>):WeightRecord{return{id:Number(row.id),occurredAt:String(row.occurred_at),occurredHasExplicitTime:Boolean(row.occurred_has_explicit_time),weightKg:Number(row.weight_kg),source:row.source as WeightRecord["source"],healthKitSampleId:row.healthkit_sample_id?String(row.healthkit_sample_id):null,healthKitSourceBundle:row.healthkit_source_bundle?String(row.healthkit_source_bundle):null,healthKitSourceName:row.healthkit_source_name?String(row.healthkit_source_name):null,healthKitSyncIdentifier:row.healthkit_sync_identifier?String(row.healthkit_sync_identifier):null,healthKitSyncVersion:Number(row.healthkit_sync_version??1),createdAt:String(row.created_at),updatedAt:String(row.updated_at)};}
export function listWeightRecords(input:import("./repositories/types").WeightRecordListInput={}){const conditions=["user_id='local'"];const values:Array<string|number>=[];if(input.from){conditions.push("occurred_at>=?");values.push(input.from);}if(input.to){conditions.push("occurred_at<?");values.push(input.to);}const limit=Math.min(Math.max(input.limit??100,1),2000);return(database.prepare(`SELECT * FROM weight_records WHERE ${conditions.join(" AND ")} ORDER BY occurred_at DESC,id DESC LIMIT ?`).all(...values,limit) as Record<string,unknown>[]).map(weightRecordFromRow);}
export function getWeightRecord(id:number){const row=database.prepare("SELECT * FROM weight_records WHERE id=? AND user_id='local'").get(id) as Record<string,unknown>|undefined;return row?weightRecordFromRow(row):null;}
export function createWeightRecord(input:import("./repositories/types").NewWeightRecord){const syncIdentifier=`evaorbit.weight.${crypto.randomUUID()}`;const result=database.prepare("INSERT INTO weight_records(user_id,occurred_at,occurred_has_explicit_time,weight_kg,source,healthkit_sync_identifier) VALUES('local',?,?,?,'manual',?)").run(input.occurredAt,Number(input.occurredHasExplicitTime),input.weightKg,syncIdentifier);return getWeightRecord(Number(result.lastInsertRowid))!;}
export function updateWeightRecord(id:number,input:import("./repositories/types").WeightRecordPatch){const current=getWeightRecord(id);if(!current)return null;const occurredAt=input.occurredAt??current.occurredAt,hasTime=input.occurredHasExplicitTime??current.occurredHasExplicitTime,weight=input.weightKg??current.weightKg;database.prepare("UPDATE weight_records SET occurred_at=?,occurred_has_explicit_time=?,weight_kg=?,healthkit_sync_version=healthkit_sync_version+1,updated_at=CURRENT_TIMESTAMP WHERE id=? AND user_id='local'").run(occurredAt,Number(hasTime),weight,id);return getWeightRecord(id);}
export function deleteWeightRecord(id:number){return database.prepare("DELETE FROM weight_records WHERE id=? AND user_id='local'").run(id).changes>0;}
...
function menstrualFlowFromRow(row:Record<string,unknown>):MenstrualFlowRecord{return{id:Number(row.id),periodId:row.period_id===null?null:Number(row.period_id),occurredAt:String(row.occurred_at),endedAt:String(row.ended_at??row.occurred_at),occurredHasExplicitTime:Boolean(row.occurred_has_explicit_time),flow:row.flow as MenstrualFlowRecord["flow"],isCycleStart:Boolean(row.is_cycle_start),notes:String(row.notes??""),source:(row.source??"manual") as MenstrualFlowRecord["source"],healthKitSampleId:row.healthkit_sample_id?String(row.healthkit_sample_id):null,healthKitSourceBundle:row.healthkit_source_bundle?String(row.healthkit_source_bundle):null,healthKitSourceName:row.healthkit_source_name?String(row.healthkit_source_name):null,healthKitSyncIdentifier:row.healthkit_sync_identifier?String(row.healthkit_sync_identifier):null,healthKitSyncVersion:Number(row.healthkit_sync_version??1),healthKitSyncStatus:(row.healthkit_sync_status??"pending") as MenstrualFlowRecord["healthKitSyncStatus"],deletedAt:row.deleted_at?String(row.deleted_at):null,createdAt:String(row.created_at),updatedAt:String(row.updated_at)};}
export function listMenstrualFlowRecords(input:import("./repositories/types").MenstrualFlowRecordListInput={}){const conditions=["user_id='local'"],values:Array<string|number>=[];if(!input.includeDeleted)conditions.push("deleted_at IS NULL");if(input.periodId){conditions.push("period_id=?");values.push(input.periodId);}if(input.from){conditions.push("occurred_at>=?");values.push(input.from);}if(input.to){conditions.push("occurred_at<?");values.push(input.to);}const limit=Math.min(Math.max(input.limit??100,1),500);return(database.prepare(`SELECT * FROM menstrual_flow_records WHERE ${conditions.join(" AND ")} ORDER BY occurred_at DESC,id DESC LIMIT ?`).all(...values,limit) as Record<string,unknown>[]).map(menstrualFlowFromRow);}
...
export function createMenstrualFlowRecord(input:import("./repositories/types").NewMenstrualFlowRecord){const syncIdentifier=`evaorbit.menstrual_flow.${crypto.randomUUID()}`;const result=database.prepare("INSERT INTO menstrual_flow_records(user_id,period_id,occurred_at,ended_at,occurred_has_explicit_time,flow,is_cycle_start,notes,source,healthkit_sync_identifier,healthkit_sync_status) VALUES('local',?,?,?,?,?,?,?,'manual',?,'pending')").run(input.periodId,input.occurredAt,input.occurredAt,Number(input.occurredHasExplicitTime),input.flow,Number(input.isCycleStart),input.notes,syncIdentifier);return getMenstrualFlowRecord(Number(result.lastInsertRowid))!;}
export function updateMenstrualFlowRecord(id:number,input:import("./repositories/types").MenstrualFlowRecordPatch){const current=getMenstrualFlowRecord(id);if(!current||current.deletedAt)return null;if(current.source==="apple_health")throw new ConflictError("请在 Apple Health 中修改这条导入记录");const periodId=input.periodId===undefined?current.periodId:input.periodId,isCycleStart=input.isCycleStart??current.isCycleStart;if(isCycleStart&&periodId===null)throw new ConflictError("周期开始记录必须关联经期");const map={periodId:"period_id",occurredAt:"occurred_at",occurredHasExplicitTime:"occurred_has_explicit_time",flow:"flow",isCycleStart:"is_cycle_start",notes:"notes"} as const;const entries=Object.entries(map).filter(([key])=>input[key as keyof typeof input]!==undefined);if(!entries.length)return current;const value=(key:string)=>key==="occurredHasExplicitTime"||key==="isCycleStart"?Number(input[key as keyof typeof input]):input[key as keyof typeof input] as string|number|null;const occurredAt=input.occurredAt??current.occurredAt;const result=database.prepare(`UPDATE menstrual_flow_records SET ${entries.map(([,column])=>`${column}=?`).join(",")},ended_at=?,healthkit_sync_version=healthkit_sync_version+1,healthkit_sync_status='pending',healthkit_sample_id=NULL,updated_at=CURRENT_TIMESTAMP WHERE id=? AND user_id='local'`).run(...entries.map(([key])=>value(key)),occurredAt,id);return result.changes?getMenstrualFlowRecord(id):null;}
export function deleteMenstrualFlowRecord(id:number){const current=getMenstrualFlowRecord(id);if(!current)return false;if(current.source==="apple_health")throw new ConflictError("请在 Apple Health 中删除这条导入记录");if(!current.healthKitSampleId&&!current.healthKitSyncIdentifier)return finalizeMenstrualFlowRecordDeletion(id);return database.prepare("UPDATE menstrual_flow_records SET deleted_at=CURRENT_TIMESTAMP,healthkit_sync_status='pending_delete',updated_at=CURRENT_TIMESTAMP WHERE id=? AND user_id='local'").run(id).changes>0;}
export function finalizeMenstrualFlowRecordDeletion(id:number){return database.prepare("DELETE FROM menstrual_flow_records WHERE id=? AND user_id='local'").run(id).changes>0;}
...
export function listMediaItems(input:MediaListInput={}){const conditions:string[]=["mi.user_id='local'"],values:Array<string|number>=[];if(input.query){conditions.push("(mi.title LIKE ? OR mi.original_title LIKE ? OR mi.translated_title LIKE ? OR mi.season_title LIKE ? OR ms.name LIKE ?)");values.push(...Array(5).fill(`%${input.query}%`));}if(input.mediaType){conditions.push("mi.media_type=?");values.push(input.mediaType);}if(input.status){conditions.push("mi.status=?");values.push(input.status);}if(input.rating){conditions.push("mi.rating=?");values.push(input.rating);}if(input.seriesId){conditions.push("mi.series_id=?");values.push(input.seriesId);}if(input.favorite!==undefined){conditions.push("mi.is_favorite=?");values.push(Number(input.favorite));}const limit=Math.min(Math.max(input.limit??100,1),200);return(database.prepare(`SELECT mi.*,ms.name AS series_name FROM media_items mi LEFT JOIN media_series ms ON ms.id=mi.series_id AND ms.user_id=mi.user_id WHERE ${conditions.join(" AND ")} ORDER BY mi.updated_at DESC,mi.id DESC LIMIT ?`).all(...values,limit) as Record<string,unknown>[]).map(mediaItemFromRow);}
export function getMediaItem(id:number){const row=database.prepare("SELECT mi.*,ms.name AS series_name FROM media_items mi LEFT JOIN media_series ms ON ms.id=mi.series_id AND ms.user_id=mi.user_id WHERE mi.id=? AND mi.user_id='local'").get(id) as Record<string,unknown>|undefined;return row?mediaItemFromRow(row):null;}
export function listMediaSeries(){return(database.prepare("SELECT ms.*,COUNT(mi.id) AS item_count FROM media_series ms LEFT JOIN media_items mi ON mi.series_id=ms.id AND mi.user_id=ms.user_id WHERE ms.user_id='local' GROUP BY ms.id ORDER BY ms.name COLLATE NOCASE").all() as Record<string,unknown>[]).map(mediaSeriesFromRow);}
export function getMediaSeries(id:number){const row=database.prepare("SELECT ms.*,COUNT(mi.id) AS item_count FROM media_series ms LEFT JOIN media_items mi ON mi.series_id=ms.id AND mi.user_id=ms.user_id WHERE ms.id=? AND ms.user_id='local' GROUP BY ms.id").get(id) as Record<string,unknown>|undefined;return row?mediaSeriesFromRow(row):null;}
export function createMediaSeries(name:string){const existing=database.prepare("SELECT id FROM media_series WHERE user_id='local' AND name=? COLLATE NOCASE").get(name) as {id:number}|undefined;if(existing)return getMediaSeries(existing.id)!;const result=database.prepare("INSERT INTO media_series(user_id,name) VALUES('local',?)").run(name);return getMediaSeries(Number(result.lastInsertRowid))!;}
export function createMediaItem(input:NewMediaItem){const result=database.prepare("INSERT INTO media_items(user_id,title,original_title,translated_title,media_type,status,rating,is_favorite,note,cover_url,series_id,season_number,season_title) VALUES('local',?,?,?,?,?,?,?,?,?,?,?,?)").run(input.title,input.originalTitle,input.translatedTitle,input.mediaType,input.status,input.rating,Number(input.isFavorite),input.note,input.coverUrl,input.seriesId,input.seasonNumber,input.seasonTitle);return getMediaItem(Number(result.lastInsertRowid))!;}
export function updateMediaItem(id:number,input:MediaItemPatch){const map:Record<string,string>={title:"title",originalTitle:"original_title",translatedTitle:"translated_title",mediaType:"media_type",status:"status",rating:"rating",isFavorite:"is_favorite",note:"note",coverUrl:"cover_url",seriesId:"series_id",seasonNumber:"season_number",seasonTitle:"season_title"};const entries=Object.entries(map).filter(([key])=>input[key as keyof MediaItemPatch]!==undefined);if(!entries.length)return getMediaItem(id);const value=(key:string)=>key==="isFavorite"?Number(input.isFavorite):input[key as keyof MediaItemPatch] as string|number|null;const result=database.prepare(`UPDATE media_items SET ${entries.map(([,column])=>`${column}=?`).join(",")},updated_at=CURRENT_TIMESTAMP WHERE id=? AND user_id='local'`).run(...entries.map(([key])=>value(key)),id);return result.changes?getMediaItem(id):null;}
...
function luciusPostFromRow(row:Record<string,unknown>):LuciusPost{return{id:Number(row.id),content:String(row.content),publishedAt:String(row.published_at),createdAt:String(row.created_at),updatedAt:String(row.updated_at)};}
export function listLuciusPosts(input:LuciusPostListInput={}){const limit=Math.min(Math.max(input.limit??50,1),100);return(database.prepare("SELECT * FROM lucius_posts WHERE user_id='local' ORDER BY published_at DESC,id DESC LIMIT ?").all(limit) as Record<string,unknown>[]).map(luciusPostFromRow);}
export function getLuciusPost(id:number){const row=database.prepare("SELECT * FROM lucius_posts WHERE id=? AND user_id='local'").get(id) as Record<string,unknown>|undefined;return row?luciusPostFromRow(row):null;}
...
export function createDrinkLog(input:Omit<DrinkLog,"id"|"createdAt"|"updatedAt">){const result=database.prepare("INSERT INTO drink_logs(occurred_at,occurred_has_explicit_time,name,brand,drink_type,volume_ml,sugar_level,temperature,rating,caffeine_mg,estimated_kcal,kcal_min,kcal_max,confidence,food_library_id,notes) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)").run(input.occurredAt,Number(input.occurredHasExplicitTime),input.name,input.brand,input.drinkType,input.volumeMl,input.sugarLevel,input.temperature,input.rating,input.caffeineMg,input.estimatedKcal,input.kcalMin,input.kcalMax,input.confidence,input.foodLibraryId,input.notes);return getDrinkLog(Number(result.lastInsertRowid))!;}
export function updateDrinkLog(id:number,input:Record<string,unknown>){const map:Record<string,string>={occurredAt:"occurred_at",occurredHasExplicitTime:"occurred_has_explicit_time",name:"name",brand:"brand",drinkType:"drink_type",volumeMl:"volume_ml",sugarLevel:"sugar_level",temperature:"temperature",rating:"rating",caffeineMg:"caffeine_mg",estimatedKcal:"estimated_kcal",kcalMin:"kcal_min",kcalMax:"kcal_max",confidence:"confidence",foodLibraryId:"food_library_id",notes:"notes"};const entries=Object.entries(map).filter(([key])=>input[key]!==undefined);if(!entries.length)return getDrinkLog(id);database.prepare(`UPDATE drink_logs SET ${entries.map(([,column])=>`${column}=?`).join(",")},updated_at=CURRENT_TIMESTAMP WHERE id=?`).run(...entries.map(([key])=>key==="occurredHasExplicitTime"?Number(input[key]):input[key] as string|number|null),id);return getDrinkLog(id);}
...
function trackerReminderFromRow(row:Record<string,unknown>):TrackerReminder{return{id:Number(row.id),trackerId:Number(row.tracker_id),reminderMode:(row.notification_mode??(row.reminder_type==="scheduled"?"standard":"missing")) as TrackerReminder["reminderMode"],configuredTime:String(row.configured_time??row.schedule_rule??"20:00").slice(0,5),periodDays:Number(row.period_days??row.interval_days??1),anchorDate:String(row.anchor_date??"1970-01-01"),nextDueAt:String(row.next_due_at),timezone:String(row.timezone??"Asia/Shanghai"),reminderId:row.reminder_id===null||row.reminder_id===undefined?null:Number(row.reminder_id),enabled:Boolean(row.enabled),createdAt:String(row.created_at),updatedAt:String(row.updated_at)};}
...
export function getTrackerReminder(id:number){const row=database.prepare("SELECT * FROM tracker_reminders WHERE id=?").get(id) as Record<string,unknown>|undefined;return row?trackerReminderFromRow(row):null;}
export function createTrackerReminder(input:Omit<TrackerReminder,"id"|"createdAt"|"updatedAt">){const legacyType=input.reminderMode==="standard"?"scheduled":"interval";const result=database.prepare("INSERT INTO tracker_reminders(tracker_id,reminder_type,schedule_rule,interval_days,notification_mode,configured_time,period_days,anchor_date,next_due_at,timezone,reminder_id,enabled) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)").run(input.trackerId,legacyType,input.configuredTime,input.periodDays,input.reminderMode,input.configuredTime,input.periodDays,input.anchorDate,input.nextDueAt,input.timezone,input.reminderId,Number(input.enabled));return getTrackerReminder(Number(result.lastInsertRowid))!;}
export function updateTrackerReminder(id:number,input:Record<string,unknown>){updateCatsRow("tracker_reminders",id,input,{reminderMode:"notification_mode",configuredTime:"configured_time",periodDays:"period_days",anchorDate:"anchor_date",nextDueAt:"next_due_at",timezone:"timezone",reminderId:"reminder_id",enabled:"enabled"},[],["enabled"]);return getTrackerReminder(id);}
...
function catRoutineFromRow(row:Record<string,unknown>):CatRoutine{return{id:Number(row.id),scope:row.scope as CatRoutine["scope"],petId:row.pet_id===null?null:Number(row.pet_id),title:String(row.title),intervalValue:Number(row.interval_value),intervalUnit:row.interval_unit as CatRoutine["intervalUnit"],recurrenceMode:(row.recurrence_mode??"completion") as CatRoutine["recurrenceMode"],anchorDate:String(row.anchor_date),firstDueDate:String(row.first_due_date),nextDueDate:String(row.next_due_date),configuredReminderTime:String(row.configured_reminder_time).slice(0,5),timezone:String(row.timezone??"Asia/Shanghai"),firstDueAt:String(row.first_due_at),lastCompletedAt:row.last_completed_at?String(row.last_completed_at):null,nextDueAt:String(row.next_due_at),reminderLeadMinutes:Number(row.reminder_lead_minutes??0),repeatWhileOverdue:Boolean(row.repeat_while_overdue),notes:String(row.notes??""),enabled:Boolean(row.enabled),reminderId:row.reminder_id===null?null:Number(row.reminder_id),createdAt:String(row.created_at),updatedAt:String(row.updated_at)};}
function reminderFromRow(row:Record<string,unknown>):Reminder{return{id:Number(row.id),title:String(row.title),targetType:row.target_type as Reminder["targetType"],targetId:row.target_id===null?null:Number(row.target_id),sourceType:row.source_type?String(row.source_type):null,sourceId:row.source_id===null?null:Number(row.source_id),scheduleType:row.schedule_type as Reminder["scheduleType"],startsAt:String(row.starts_at),nextDueAt:row.next_due_at?String(row.next_due_at):null,overdueAfter:row.overdue_after?String(row.overdue_after):null,dueHasExplicitTime:row.due_has_explicit_time===undefined?true:Boolean(row.due_has_explicit_time),intervalValue:row.interval_value===null?null:Number(row.interval_value),intervalUnit:row.interval_unit as Reminder["intervalUnit"],timesOfDay:jsonValue(row.times_of_day,[]),endsAt:row.ends_at?String(row.ends_at):null,timezone:String(row.timezone),note:String(row.note??""),leadTimeMinutes:Number(row.lead_time_minutes??0),repeatWhileOverdue:Boolean(row.repeat_while_overdue),status:(row.status??"scheduled") as Reminder["status"],sentAt:row.sent_at?String(row.sent_at):null,cancelledAt:row.cancelled_at?String(row.cancelled_at):null,isActive:Boolean(row.is_active),lastCompletedAt:row.last_completed_at?String(row.last_completed_at):null,snoozedUntil:row.snoozed_until?String(row.snoozed_until):null,lastNotifiedAt:row.last_notified_at?String(row.last_notified_at):null,createdAt:String(row.created_at),updatedAt:String(row.updated_at)};}
...
export function createCatRoutine(input:Omit<CatRoutine,"id"|"lastCompletedAt"|"createdAt"|"updatedAt">){const r=database.prepare("INSERT INTO cat_routines(scope,pet_id,title,interval_value,interval_unit,recurrence_mode,anchor_date,first_due_date,next_due_date,configured_reminder_time,timezone,first_due_at,next_due_at,reminder_lead_minutes,repeat_while_overdue,notes,enabled,reminder_id) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)").run(input.scope,input.petId,input.title,input.intervalValue,input.intervalUnit,input.recurrenceMode,input.anchorDate,input.firstDueDate,input.nextDueDate,input.configuredReminderTime,input.timezone,input.firstDueAt,input.nextDueAt,input.reminderLeadMinutes,Number(input.repeatWhileOverdue),input.notes,Number(input.enabled),input.reminderId);return getCatRoutine(Number(r.lastInsertRowid))!;}
export function updateCatRoutine(id:number,input:Record<string,unknown>){updateCatsRow("cat_routines",id,input,{scope:"scope",petId:"pet_id",title:"title",intervalValue:"interval_value",intervalUnit:"interval_unit",recurrenceMode:"recurrence_mode",anchorDate:"anchor_date",firstDueDate:"first_due_date",nextDueDate:"next_due_date",configuredReminderTime:"configured_reminder_time",timezone:"timezone",firstDueAt:"first_due_at",lastCompletedAt:"last_completed_at",nextDueAt:"next_due_at",reminderLeadMinutes:"reminder_lead_minutes",repeatWhileOverdue:"repeat_while_overdue",notes:"notes",enabled:"enabled",reminderId:"reminder_id"},[],["enabled","repeatWhileOverdue"]);return getCatRoutine(id);}
...
export function createReminder(input:Omit<Reminder,"id"|"overdueAfter"|"lastCompletedAt"|"snoozedUntil"|"lastNotifiedAt"|"sentAt"|"cancelledAt"|"createdAt"|"updatedAt">&{overdueAfter?:string|null}){const r=database.prepare("INSERT INTO reminders(title,target_type,target_id,source_type,source_id,schedule_type,starts_at,next_due_at,overdue_after,due_has_explicit_time,interval_value,interval_unit,times_of_day,ends_at,timezone,note,lead_time_minutes,repeat_while_overdue,status,is_active) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)").run(input.title,input.targetType,input.targetId,input.sourceType,input.sourceId,input.scheduleType,input.startsAt,input.nextDueAt,input.overdueAfter??null,Number(input.dueHasExplicitTime),input.intervalValue,input.intervalUnit,JSON.stringify(input.timesOfDay),input.endsAt,input.timezone,input.note,input.leadTimeMinutes,Number(input.repeatWhileOverdue),input.status,Number(input.isActive));return getReminder(Number(r.lastInsertRowid))!;}
export function updateReminder(id:number,input:Record<string,unknown>){updateCatsRow("reminders",id,input,{title:"title",targetType:"target_type",targetId:"target_id",sourceType:"source_type",sourceId:"source_id",scheduleType:"schedule_type",startsAt:"starts_at",nextDueAt:"next_due_at",overdueAfter:"overdue_after",dueHasExplicitTime:"due_has_explicit_time",intervalValue:"interval_value",intervalUnit:"interval_unit",timesOfDay:"times_of_day",endsAt:"ends_at",timezone:"timezone",note:"note",leadTimeMinutes:"lead_time_minutes",repeatWhileOverdue:"repeat_while_overdue",status:"status",sentAt:"sent_at",cancelledAt:"cancelled_at",isActive:"is_active",lastCompletedAt:"last_completed_at",snoozedUntil:"snoozed_until",lastNotifiedAt:"last_notified_at"},["timesOfDay"],["isActive","dueHasExplicitTime","repeatWhileOverdue"]);return getReminder(id);}
...
export function listRelationPeople(input:{query?:string;includeArchived?:boolean;relationshipStatus?:"active"|"ended"}={}){const clauses=[input.includeArchived?"1=1":"archived_at IS NULL"];const args:(string|number|null)[]=[];if(input.query){clauses.push("(name LIKE ? OR nickname LIKE ? OR relation_label LIKE ?)");const q=`%${input.query}%`;args.push(q,q,q);}if(input.relationshipStatus){clauses.push("relationship_status=?");args.push(input.relationshipStatus);}return(database.prepare(`SELECT * FROM relation_people WHERE ${clauses.join(" AND ")} ORDER BY updated_at DESC,id DESC`).all(...args) as Record<string,unknown>[]).map(relationPersonFromRow);}
...
export function createRelationPerson(input:NewRelationPerson){const r=database.prepare("INSERT INTO relation_people(name,nickname,relation_label,closeness_rank,relationship_status,birthday,likes,avoid,note) VALUES(?,?,?,?,?,?,?,?,?)").run(input.name,input.nickname,input.relationLabel,input.closenessRank,input.relationshipStatus,input.birthday,input.likes,input.avoid,input.note);return getRelationPerson(Number(r.lastInsertRowid))!;}
export function updateRelationPerson(id:number,input:RelationPersonPatch){const current=getRelationPerson(id);if(!current)return null;const next={...current,...input,updatedAt:new Date().toISOString()};database.prepare("UPDATE relation_people SET name=?,nickname=?,relation_label=?,closeness_rank=?,relationship_status=?,photo_path=?,birthday=?,likes=?,avoid=?,note=?,archived_at=?,updated_at=? WHERE id=?").run(next.name,next.nickname,next.relationLabel,next.closenessRank,next.relationshipStatus,next.photoPath,next.birthday,next.likes,next.avoid,next.note,next.archivedAt,next.updatedAt,id);return getRelationPerson(id);}
...
export function listMealReminderRules(){return(database.prepare("SELECT * FROM meal_reminder_rules ORDER BY CASE meal_type WHEN 'breakfast' THEN 1 WHEN 'lunch' THEN 2 ELSE 3 END").all() as Record<string,unknown>[]).map(mealReminderRuleFromRow);}
export function updateMealReminderRules(input:Array<Pick<MealReminderRule,"mealType"|"remindAt"|"enabled">>){const update=database.prepare("UPDATE meal_reminder_rules SET remind_at=?,enabled=?,updated_at=CURRENT_TIMESTAMP WHERE meal_type=?");for(const rule of input)update.run(rule.remindAt,Number(rule.enabled),rule.mealType);return listMealReminderRules();}
...
function subscriptionPaymentFromRow(row:Record<string,unknown>):SubscriptionPayment{return{id:Number(row.id),subscriptionId:Number(row.subscription_id),scheduledFor:String(row.scheduled_for),paidOn:String(row.paid_on),amountMinor:Number(row.amount_minor),currency:String(row.currency),note:String(row.note??""),createdAt:String(row.created_at)};}
function subscriptionPriceChangeFromRow(row:Record<string,unknown>):SubscriptionPriceChange{return{id:Number(row.id),subscriptionId:Number(row.subscription_id),effectiveOn:String(row.effective_on),oldAmountMinor:Number(row.old_amount_minor),newAmountMinor:Number(row.new_amount_minor),oldCurrency:String(row.old_currency),newCurrency:String(row.new_currency),createdAt:String(row.created_at)};}
function subscriptionFromRow(row:Record<string,unknown>):Subscription{const spendByCurrency=JSON.parse(String(row.spend_by_currency??"{}")) as Record<string,number>;return{id:Number(row.id),name:String(row.name),currentAmountMinor:Number(row.current_amount_minor),currency:String(row.currency),billingIntervalValue:Number(row.billing_interval_value),billingIntervalUnit:row.billing_interval_unit as Subscription["billingIntervalUnit"],startedOn:String(row.started_on),nextRenewalOn:String(row.next_renewal_on),status:row.status as Subscription["status"],autoRenew:Boolean(row.auto_renew),reminderEnabled:Boolean(row.reminder_enabled),reminderDaysBefore:Number(row.reminder_days_before),reminderTime:row.reminder_time?String(row.reminder_time).slice(0,5):null,reminderId:row.reminder_id===null?null:Number(row.reminder_id),notes:String(row.notes??""),pausedAt:row.paused_at?String(row.paused_at):null,endedAt:row.ended_at?String(row.ended_at):null,totalSpentMinor:Number(spendByCurrency[String(row.currency)]??0),spendByCurrency,paymentCount:Number(row.payment_count??0),latestPriceChange:row.latest_change_id?subscriptionPriceChangeFromRow({id:row.latest_change_id,subscription_id:row.id,effective_on:row.latest_change_effective_on,old_amount_minor:row.latest_change_old_amount_minor,new_amount_minor:row.latest_change_new_amount_minor,old_currency:row.latest_change_old_currency,new_currency:row.latest_change_new_currency,created_at:row.latest_change_created_at}):null,createdAt:String(row.created_at),updatedAt:String(row.updated_at)};}
const subscriptionSelect=`SELECT s.*,(SELECT json_group_object(currency,total_minor) FROM (SELECT p.currency,SUM(p.amount_minor) total_minor FROM subscription_payments p WHERE p.subscription_id=s.id GROUP BY p.currency ORDER BY p.currency)) spend_by_currency,(SELECT COUNT(*) FROM subscription_payments p WHERE p.subscription_id=s.id) payment_count,(SELECT id FROM subscription_price_changes c WHERE c.subscription_id=s.id ORDER BY effective_on DESC,id DESC LIMIT 1) latest_change_id,(SELECT effective_on FROM subscription_price_changes c WHERE c.subscription_id=s.id ORDER BY effective_on DESC,id DESC LIMIT 1) latest_change_effective_on,(SELECT old_amount_minor FROM subscription_price_changes c WHERE c.subscription_id=s.id ORDER BY effective_on DESC,id DESC LIMIT 1) latest_change_old_amount_minor,(SELECT new_amount_minor FROM subscription_price_changes c WHERE c.subscription_id=s.id ORDER BY effective_on DESC,id DESC LIMIT 1) latest_change_new_amount_minor,(SELECT old_currency FROM subscription_price_changes c WHERE c.subscription_id=s.id ORDER BY effective_on DESC,id DESC LIMIT 1) latest_change_old_currency,(SELECT new_currency FROM subscription_price_changes c WHERE c.subscription_id=s.id ORDER BY effective_on DESC,id DESC LIMIT 1) latest_change_new_currency,(SELECT created_at FROM subscription_price_changes c WHERE c.subscription_id=s.id ORDER BY effective_on DESC,id DESC LIMIT 1) latest_change_created_at FROM subscriptions s`;
export function listSubscriptions(input:SubscriptionListInput={}){const conditions:string[]=[],values:Array<string|number>=[];if(input.status){conditions.push("s.status=?");values.push(input.status);}if(input.query){conditions.push("s.name LIKE ?");values.push(`%${input.query}%`);}values.push(Math.min(Math.max(input.limit??200,1),500));return(database.prepare(`${subscriptionSelect}${conditions.length?` WHERE ${conditions.join(" AND ")}`:""} ORDER BY CASE s.status WHEN 'active' THEN 1 WHEN 'paused' THEN 2 ELSE 3 END,s.next_renewal_on,s.id LIMIT ?`).all(...values) as Record<string,unknown>[]).map(subscriptionFromRow);}
export function getSubscription(id:number){const row=database.prepare(`${subscriptionSelect} WHERE s.id=?`).get(id) as Record<string,unknown>|undefined;return row?subscriptionFromRow(row):null;}
export function createSubscription(input:NewSubscription){const r=database.prepare("INSERT INTO subscriptions(name,current_amount_minor,currency,billing_interval_value,billing_interval_unit,started_on,next_renewal_on,status,auto_renew,reminder_enabled,reminder_days_before,reminder_time,notes) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)").run(input.name,input.currentAmountMinor,input.currency,input.billingIntervalValue,input.billingIntervalUnit,input.startedOn,input.nextRenewalOn,input.status??"active",Number(input.autoRenew),Number(input.reminderEnabled),input.reminderDaysBefore,input.reminderTime,input.notes);return getSubscription(Number(r.lastInsertRowid))!;}
export function updateSubscription(id:number,input:SubscriptionPatch,effectiveOn=new Date().toISOString().slice(0,10)){const current=getSubscription(id);if(!current)return null;database.exec("BEGIN");try{if((input.currentAmountMinor!==undefined&&input.currentAmountMinor!==current.currentAmountMinor)||(input.currency!==undefined&&input.currency!==current.currency))database.prepare("INSERT INTO subscription_price_changes(subscription_id,effective_on,old_amount_minor,new_amount_minor,old_currency,new_currency) VALUES(?,?,?,?,?,?)").run(id,effectiveOn,current.currentAmountMinor,input.currentAmountMinor??current.currentAmountMinor,current.currency,input.currency??current.currency);updateCatsRow("subscriptions",id,input,{name:"name",currentAmountMinor:"current_amount_minor",currency:"currency",billingIntervalValue:"billing_interval_value",billingIntervalUnit:"billing_interval_unit",startedOn:"started_on",nextRenewalOn:"next_renewal_on",autoRenew:"auto_renew",reminderEnabled:"reminder_enabled",reminderDaysBefore:"reminder_days_before",reminderTime:"reminder_time",reminderId:"reminder_id",notes:"notes"},[],["autoRenew","reminderEnabled"]);database.exec("COMMIT");return getSubscription(id);}catch(error){database.exec("ROLLBACK");throw error;}}
export function setSubscriptionStatus(id:number,status:Subscription["status"],nextRenewalOn?:string){const now=new Date().toISOString();database.prepare("UPDATE subscriptions SET status=?,next_renewal_on=COALESCE(?,next_renewal_on),paused_at=?,ended_at=?,updated_at=? WHERE id=?").run(status,nextRenewalOn??null,status==="paused"?now:null,status==="ended"?now:null,now,id);return getSubscription(id);}
export function listSubscriptionPayments(subscriptionId:number){return(database.prepare("SELECT * FROM subscription_payments WHERE subscription_id=? ORDER BY paid_on DESC,id DESC").all(subscriptionId) as Record<string,unknown>[]).map(subscriptionPaymentFromRow);}
export function listSubscriptionPriceChanges(subscriptionId:number){return(database.prepare("SELECT * FROM subscription_price_changes WHERE subscription_id=? ORDER BY effective_on DESC,id DESC").all(subscriptionId) as Record<string,unknown>[]).map(subscriptionPriceChangeFromRow);}
export function recordSubscriptionPayment(subscriptionId:number,input:NewSubscriptionPayment,nextRenewalOn:string){const current=getSubscription(subscriptionId);if(!current||current.status!=="active")throw new ConflictError("订阅不存在或未启用");database.exec("BEGIN");try{if(input.updateCurrentPrice&&(input.amountMinor!==current.currentAmountMinor||input.currency!==current.currency))database.prepare("INSERT INTO subscription_price_changes(subscription_id,effective_on,old_amount_minor,new_amount_minor,old_currency,new_currency) VALUES(?,?,?,?,?,?)").run(subscriptionId,input.paidOn,current.currentAmountMinor,input.amountMinor,current.currency,input.currency);const r=database.prepare("INSERT INTO subscription_payments(subscription_id,scheduled_for,paid_on,amount_minor,currency,note) VALUES(?,?,?,?,?,?)").run(subscriptionId,input.scheduledFor,input.paidOn,input.amountMinor,input.currency,input.note);database.prepare("UPDATE subscriptions SET next_renewal_on=?,current_amount_minor=?,currency=?,updated_at=CURRENT_TIMESTAMP WHERE id=?").run(nextRenewalOn,input.updateCurrentPrice?input.amountMinor:current.currentAmountMinor,input.updateCurrentPrice?input.currency:current.currency,subscriptionId);database.exec("COMMIT");return subscriptionPaymentFromRow(database.prepare("SELECT * FROM subscription_payments WHERE id=?").get(Number(r.lastInsertRowid)) as Record<string,unknown>);}catch(error){database.exec("ROLLBACK");if(error instanceof Error&&error.message.includes("UNIQUE constraint failed"))throw new ConflictError("这个账期已经记录过扣款");throw error;}}

function memoryEntityFromRow(row:Record<string,unknown>):MemoryEntity{return{id:String(row.id),canonicalName:String(row.canonical_name),entityType:String(row.entity_type),aliases:stringArrayFromRow(row.aliases),description:row.description===null?null:String(row.description),status:row.status as MemoryEntity["status"],mergedIntoEntityId:row.merged_into_entity_id===null?null:String(row.merged_into_entity_id),createdAt:String(row.created_at),updatedAt:String(row.updated_at)};}
function memoryFactFromRow(row:Record<string,unknown>):MemoryFact{return{id:String(row.id),subjectEntityId:String(row.subject_entity_id),predicate:String(row.predicate),objectEntityId:row.object_entity_id===null?null:String(row.object_entity_id),objectValue:row.object_value===null?null:jsonValue(row.object_value,null),perspectiveEntityId:row.perspective_entity_id===null?null:String(row.perspective_entity_id),epistemicType:(row.epistemic_type??"unknown") as MemoryFact["epistemicType"],supersedesFactId:row.supersedes_fact_id===null||row.supersedes_fact_id===undefined?null:String(row.supersedes_fact_id),confidence:Number(row.confidence),importance:Number(row.importance),validFrom:row.valid_from===null?null:String(row.valid_from),validTo:row.valid_to===null?null:String(row.valid_to),status:row.status as MemoryFact["status"],invalidatedAt:row.invalidated_at===null?null:String(row.invalidated_at),invalidationReason:row.invalidation_reason===null?null:String(row.invalidation_reason),createdAt:String(row.created_at),updatedAt:String(row.updated_at)};}
function memorySourceFromRow(row:Record<string,unknown>):MemorySource{return{id:String(row.id),factId:String(row.fact_id),sourceResource:String(row.source_resource),sourceRecordId:row.source_record_id===null?null:String(row.source_record_id),sourceUrl:row.source_url===null?null:String(row.source_url),excerpt:row.excerpt===null?null:String(row.excerpt),note:row.note===null?null:String(row.note),createdAt:String(row.created_at),updatedAt:String(row.updated_at)};}
function memoryFactCandidateFromRow(row:Record<string,unknown>):MemoryFactCandidate{return{id:String(row.id),proposedFact:jsonValue(row.proposed_fact,{}) as MemoryFactCandidate["proposedFact"],proposedSources:jsonValue(row.proposed_sources,[]) as MemoryFactCandidate["proposedSources"],proposedBy:row.proposed_by as MemoryFactCandidate["proposedBy"],proposerModel:row.proposer_model===null?null:String(row.proposer_model),status:row.status as MemoryFactCandidate["status"],reviewNote:row.review_note===null?null:String(row.review_note),promotedFactId:row.promoted_fact_id===null?null:String(row.promoted_fact_id),createdAt:String(row.created_at),reviewedAt:row.reviewed_at===null?null:String(row.reviewed_at)};}

export function listMemoryEntities(input:MemoryEntityListInput={}){const conditions=["user_id='local'"],values:Array<string|number>=[];if(!input.includeMerged&&!input.status)conditions.push("status<>'merged'");if(input.status){conditions.push("status=?");values.push(input.status);}if(input.entityType){conditions.push("entity_type=?");values.push(input.entityType);}if(input.query){const pattern=`%${escapedLike(input.query.trim())}%`;conditions.push("(canonical_name LIKE ? ESCAPE '\\' COLLATE NOCASE OR EXISTS(SELECT 1 FROM json_each(memory_entities.aliases) WHERE value LIKE ? ESCAPE '\\' COLLATE NOCASE))");values.push(pattern,pattern);}const limit=Math.min(Math.max(input.limit??100,1),200);return(database.prepare(`SELECT * FROM memory_entities WHERE ${conditions.join(" AND ")} ORDER BY updated_at DESC,id LIMIT ?`).all(...values,limit) as Record<string,unknown>[]).map(memoryEntityFromRow);}
export function getMemoryEntity(id:string){const row=database.prepare("SELECT * FROM memory_entities WHERE id=? AND user_id='local'").get(id) as Record<string,unknown>|undefined;return row?memoryEntityFromRow(row):null;}
export function createMemoryEntity(input:NewMemoryEntity){database.prepare("INSERT INTO memory_entities(id,user_id,canonical_name,entity_type,aliases,description,status) VALUES(?,'local',?,?,?,?,?)").run(input.id,input.canonicalName,input.entityType,JSON.stringify(input.aliases),input.description,input.status);return getMemoryEntity(input.id)!;}
export function updateMemoryEntity(id:string,input:MemoryEntityPatch){const existing=getMemoryEntity(id);if(!existing||existing.status==="merged")return null;const map={canonicalName:"canonical_name",entityType:"entity_type",aliases:"aliases",description:"description"};const entries=Object.entries(map).filter(([key])=>input[key as keyof MemoryEntityPatch]!==undefined);if(entries.length)database.prepare(`UPDATE memory_entities SET ${entries.map(([,column])=>`${column}=?`).join(",")},updated_at=CURRENT_TIMESTAMP WHERE id=? AND user_id='local' AND status<>'merged'`).run(...entries.map(([key])=>key==="aliases"?JSON.stringify(input.aliases):input[key as keyof MemoryEntityPatch] as string|null),id);return getMemoryEntity(id);}
export function setMemoryEntityArchived(id:string,archived:boolean){const result=database.prepare("UPDATE memory_entities SET status=?,updated_at=CURRENT_TIMESTAMP WHERE id=? AND user_id='local' AND status IN ('active','archived')").run(archived?"archived":"active",id);return result.changes?getMemoryEntity(id):null;}
export function mergeMemoryEntities(sourceId:string,targetId:string){database.exec("BEGIN IMMEDIATE");try{if(sourceId===targetId)throw new ConflictError("Memory Entity source and target must differ.");const source=getMemoryEntity(sourceId),target=getMemoryEntity(targetId);if(!source||!target){database.exec("COMMIT");return null;}if(source.status!=="active"||target.status!=="active")throw new ConflictError("Both Memory Entities must be active.");const seen=new Set<string>(),mergedAliases=[...target.aliases,...source.aliases,source.canonicalName].filter(value=>{const key=value.trim().toLocaleLowerCase();if(!key||key===target.canonicalName.toLocaleLowerCase()||seen.has(key))return false;seen.add(key);return true;});if(mergedAliases.length>50)throw new ConflictError("Merged aliases would exceed the 50-alias limit.");const rows=database.prepare("SELECT id FROM memory_facts WHERE user_id='local' AND (subject_entity_id=? OR object_entity_id=? OR perspective_entity_id=?)").all(sourceId,sourceId,sourceId) as Array<{id:string}>;database.prepare("UPDATE memory_entities SET aliases=?,updated_at=CURRENT_TIMESTAMP WHERE id=? AND user_id='local'").run(JSON.stringify(mergedAliases),targetId);database.prepare("UPDATE memory_facts SET subject_entity_id=?,updated_at=CURRENT_TIMESTAMP WHERE subject_entity_id=? AND user_id='local'").run(targetId,sourceId);database.prepare("UPDATE memory_facts SET object_entity_id=?,updated_at=CURRENT_TIMESTAMP WHERE object_entity_id=? AND user_id='local'").run(targetId,sourceId);database.prepare("UPDATE memory_facts SET perspective_entity_id=?,updated_at=CURRENT_TIMESTAMP WHERE perspective_entity_id=? AND user_id='local'").run(targetId,sourceId);database.prepare("UPDATE memory_entities SET status='merged',merged_into_entity_id=?,updated_at=CURRENT_TIMESTAMP WHERE id=? AND user_id='local'").run(targetId,sourceId);const selfLoops=Number((database.prepare("SELECT count(*) count FROM memory_facts WHERE user_id='local' AND subject_entity_id=? AND object_entity_id=?").get(targetId,targetId) as {count:number}).count);database.exec("COMMIT");return{entity:getMemoryEntity(targetId)!,redirectedFacts:rows.length,selfLoops};}catch(error){database.exec("ROLLBACK");throw error;}}

export function listMemoryFacts(input:MemoryFactListInput={}){const conditions=["user_id='local'"],values:Array<string|number>=[];if(input.entityId){const direction=input.direction??"both";if(direction==="in"){conditions.push("object_entity_id=?");values.push(input.entityId);}else if(direction==="out"){conditions.push("subject_entity_id=?");values.push(input.entityId);}else{conditions.push("(subject_entity_id=? OR object_entity_id=?)");values.push(input.entityId,input.entityId);}}if(input.predicate){conditions.push("predicate=?");values.push(input.predicate);}if(input.perspectiveEntityId===null)conditions.push("perspective_entity_id IS NULL");else if(input.perspectiveEntityId){conditions.push("perspective_entity_id=?");values.push(input.perspectiveEntityId);}if(input.status){conditions.push("status=?");values.push(input.status);}if(input.validOn){conditions.push("(valid_from IS NULL OR valid_from<=?) AND (valid_to IS NULL OR valid_to>=?)");values.push(input.validOn,input.validOn);}const limit=Math.min(Math.max(input.limit??100,1),200);return(database.prepare(`SELECT * FROM memory_facts WHERE ${conditions.join(" AND ")} ORDER BY importance DESC,updated_at DESC,id LIMIT ?`).all(...values,limit) as Record<string,unknown>[]).map(memoryFactFromRow);}
export function getMemoryFact(id:string){const row=database.prepare("SELECT * FROM memory_facts WHERE id=? AND user_id='local'").get(id) as Record<string,unknown>|undefined;return row?memoryFactFromRow(row):null;}
export function createMemoryFact(input:NewMemoryFact){database.prepare("INSERT INTO memory_facts(id,user_id,subject_entity_id,predicate,object_entity_id,object_value,perspective_entity_id,epistemic_type,supersedes_fact_id,confidence,importance,valid_from,valid_to,status) VALUES(?,'local',?,?,?,?,?,?,?,?,?,?,?,?)").run(input.id,input.subjectEntityId,input.predicate,input.objectEntityId,input.objectValue===null?null:JSON.stringify(input.objectValue),input.perspectiveEntityId,input.epistemicType??"unknown",input.supersedesFactId??null,input.confidence,input.importance,input.validFrom,input.validTo,input.status);return getMemoryFact(input.id)!;}
export function createMemoryFactWithSources(input:NewMemoryFact,sources:NewMemorySource[]){if(!sources.length)throw new ConflictError("A Memory Fact requires at least one source.");database.exec("BEGIN IMMEDIATE");try{const fact=createMemoryFact(input);for(const source of sources)createMemorySource(source);database.exec("COMMIT");return fact;}catch(error){database.exec("ROLLBACK");throw error;}}
export function supersedeMemoryFact(oldFactId:string,input:NewMemoryFact,sources:NewMemorySource[]){if(!sources.length)throw new ConflictError("A Memory Fact requires at least one source.");if(input.supersedesFactId!==oldFactId)throw new ConflictError("Replacement lineage does not match the superseded Fact.");database.exec("BEGIN IMMEDIATE");try{const old=getMemoryFact(oldFactId);if(!old||old.status!=="active")throw new ConflictError("The superseded Memory Fact is missing or inactive.");const fact=createMemoryFact(input);for(const source of sources)createMemorySource(source);database.prepare("UPDATE memory_facts SET status='invalidated',invalidated_at=CURRENT_TIMESTAMP,invalidation_reason='superseded',updated_at=CURRENT_TIMESTAMP WHERE id=? AND user_id='local'").run(oldFactId);database.exec("COMMIT");return fact;}catch(error){database.exec("ROLLBACK");throw error;}}
export function updateMemoryFact(id:string,input:MemoryFactPatch){const existing=getMemoryFact(id);if(!existing)return null;const validFrom=input.validFrom===undefined?existing.validFrom:input.validFrom,validTo=input.validTo===undefined?existing.validTo:input.validTo;if(validFrom&&validTo&&validTo<validFrom)throw new ConflictError("validTo cannot be earlier than validFrom.");const entries=Object.entries({confidence:"confidence",importance:"importance",validFrom:"valid_from",validTo:"valid_to"}).filter(([key])=>input[key as keyof MemoryFactPatch]!==undefined);if(entries.length)database.prepare(`UPDATE memory_facts SET ${entries.map(([,column])=>`${column}=?`).join(",")},updated_at=CURRENT_TIMESTAMP WHERE id=? AND user_id='local'`).run(...entries.map(([key])=>input[key as keyof MemoryFactPatch] as string|number|null),id);return getMemoryFact(id);}
export function invalidateMemoryFact(id:string,reason:string|null){const result=database.prepare("UPDATE memory_facts SET status='invalidated',invalidated_at=CURRENT_TIMESTAMP,invalidation_reason=?,updated_at=CURRENT_TIMESTAMP WHERE id=? AND user_id='local' AND status='active'").run(reason,id);return result.changes?getMemoryFact(id):null;}
export function restoreMemoryFact(id:string){const replacement=database.prepare("SELECT 1 FROM memory_facts WHERE user_id='local' AND supersedes_fact_id=? AND status='active'").get(id);if(replacement)throw new ConflictError("This Memory Fact has an active replacement and cannot be restored.");const result=database.prepare("UPDATE memory_facts SET status='active',invalidated_at=NULL,invalidation_reason=NULL,updated_at=CURRENT_TIMESTAMP WHERE id=? AND user_id='local' AND status='invalidated'").run(id);return result.changes?getMemoryFact(id):null;}

export function listMemorySources(input:MemorySourceListInput={}){const conditions=["user_id='local'"],values:Array<string|number>=[];if(input.factId){conditions.push("fact_id=?");values.push(input.factId);}if(input.sourceResource){conditions.push("source_resource=?");values.push(input.sourceResource);}if(input.sourceRecordId){conditions.push("source_record_id=?");values.push(input.sourceRecordId);}const limit=Math.min(Math.max(input.limit??100,1),200);return(database.prepare(`SELECT * FROM memory_sources WHERE ${conditions.join(" AND ")} ORDER BY created_at DESC,id LIMIT ?`).all(...values,limit) as Record<string,unknown>[]).map(memorySourceFromRow);}
export function getMemorySource(id:string){const row=database.prepare("SELECT * FROM memory_sources WHERE id=? AND user_id='local'").get(id) as Record<string,unknown>|undefined;return row?memorySourceFromRow(row):null;}
export function createMemorySource(input:NewMemorySource){database.prepare("INSERT INTO memory_sources(id,user_id,fact_id,source_resource,source_record_id,source_url,excerpt,note) VALUES(?,'local',?,?,?,?,?,?)").run(input.id,input.factId,input.sourceResource,input.sourceRecordId,input.sourceUrl,input.excerpt,input.note);return getMemorySource(input.id)!;}
export function updateMemorySource(id:string,input:MemorySourcePatch){const entries=Object.entries({sourceUrl:"source_url",excerpt:"excerpt",note:"note"}).filter(([key])=>input[key as keyof MemorySourcePatch]!==undefined);if(entries.length)database.prepare(`UPDATE memory_sources SET ${entries.map(([,column])=>`${column}=?`).join(",")},updated_at=CURRENT_TIMESTAMP WHERE id=? AND user_id='local'`).run(...entries.map(([key])=>input[key as keyof MemorySourcePatch] as string|null),id);return getMemorySource(id);}
export function deleteMemorySource(id:string){return database.prepare("DELETE FROM memory_sources WHERE id=? AND user_id='local'").run(id).changes>0;}

export function listMemoryFactCandidates(input:MemoryFactCandidateListInput={}){const conditions=["user_id='local'"],values:Array<string|number>=[];if(input.status){conditions.push("status=?");values.push(input.status);}if(input.proposedBy){conditions.push("proposed_by=?");values.push(input.proposedBy);}values.push(Math.min(Math.max(input.limit??100,1),200));return(database.prepare(`SELECT * FROM memory_fact_candidates WHERE ${conditions.join(" AND ")} ORDER BY created_at DESC,id LIMIT ?`).all(...values) as Record<string,unknown>[]).map(memoryFactCandidateFromRow);}
export function getMemoryFactCandidate(id:string){const row=database.prepare("SELECT * FROM memory_fact_candidates WHERE id=? AND user_id='local'").get(id) as Record<string,unknown>|undefined;return row?memoryFactCandidateFromRow(row):null;}
export function createMemoryFactCandidate(input:NewMemoryFactCandidate){database.prepare("INSERT INTO memory_fact_candidates(id,user_id,proposed_fact,proposed_sources,proposed_by,proposer_model) VALUES(?,'local',?,?,?,?)").run(input.id,JSON.stringify(input.proposedFact),JSON.stringify(input.proposedSources),input.proposedBy,input.proposerModel);return getMemoryFactCandidate(input.id)!;}
export function promoteMemoryFactCandidate(id:string,factId:string,sourceIds:string[],reviewNote:string|null){database.exec("BEGIN IMMEDIATE");try{const candidate=getMemoryFactCandidate(id);if(!candidate||candidate.status!=="pending"){database.exec("COMMIT");return null;}const factInput={id:factId,...candidate.proposedFact,status:"active" as const};const sources=candidate.proposedSources.map((source,index)=>({id:sourceIds[index],factId,...source}));if(sources.some(source=>!source.id))throw new ConflictError("Candidate source identifiers are incomplete.");if(factInput.supersedesFactId){const old=getMemoryFact(factInput.supersedesFactId);if(!old||old.status!=="active")throw new ConflictError("The superseded Memory Fact is missing or inactive.");}createMemoryFact(factInput);for(const source of sources)createMemorySource(source);if(factInput.supersedesFactId)database.prepare("UPDATE memory_facts SET status='invalidated',invalidated_at=CURRENT_TIMESTAMP,invalidation_reason='superseded',updated_at=CURRENT_TIMESTAMP WHERE id=? AND user_id='local'").run(factInput.supersedesFactId);database.prepare("UPDATE memory_fact_candidates SET status='promoted',review_note=?,promoted_fact_id=?,reviewed_at=CURRENT_TIMESTAMP WHERE id=? AND user_id='local'").run(reviewNote,factId,id);database.exec("COMMIT");return getMemoryFactCandidate(id);}catch(error){database.exec("ROLLBACK");throw error;}}
export function rejectMemoryFactCandidate(id:string,reviewNote:string|null){const result=database.prepare("UPDATE memory_fact_candidates SET status='rejected',review_note=?,reviewed_at=CURRENT_TIMESTAMP WHERE id=? AND user_id='local' AND status='pending'").run(reviewNote,id);return result.changes?getMemoryFactCandidate(id):null;}
